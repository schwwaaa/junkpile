...
impl RendererHandle {
    ...
    pub fn select_shader(&self, shader: String) -> Result<(), String> {
        self.request(|reply| RenderCommand::SelectShader { shader, reply })
    }

    pub fn compile_source(&self, source: String, label: String) -> Result<(), String> {
        self.request(|reply| RenderCommand::CompileSource { source, label, reply })
    }

    pub fn toggle_pause(&self) -> Result<bool, String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::TogglePause { reply })?;
        response
            .recv_timeout(Duration::from_secs(8))
            .map_err(|_| "renderer did not answer the pause request".to_string())?
    }

    pub fn reset_time(&self) -> Result<(), String> {
        self.request(|reply| RenderCommand::ResetTime { reply })
    }

    pub fn export_still(&self, width: u32, height: u32, path: PathBuf) -> Result<String, String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::ExportStill { width, height, path, reply })?;
        response
            .recv_timeout(Duration::from_secs(120))
            .map_err(|_| "renderer did not finish the still export".to_string())?
    }
}
...
pub fn start(window: tauri::Window, bundle: RuntimeBundle) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-wgsl-shader-playground".into())
...
impl Renderer {
    async fn new(window: tauri::Window, bundle: RuntimeBundle) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile WGSL playground device"),
        ...
        let source_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("WGSL playground source bind group layout"),
        ...
        let source_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("WGSL playground source pipeline layout"),
        ...
        let source_uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("WGSL playground source uniforms"),
        ...
        let source_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("WGSL playground source bind group"),
        ...
        Ok(Self {
            ...
            surface_config,
            adapter_info,
            source_bind_group_layout: source_layout,
            source_pipeline_layout,
            source_pipeline,
    ...
    fn renderer_info(&self) -> RendererInfo {
        RendererInfo {
            ...
            running: true,
            time_paused: self.paused_at.is_some(),
            shader_time_seconds: self.shader_time_seconds() as f64,
    ...
    fn compile_source(&mut self, source: String, label: String) -> Result<(), String> {
        validate_editor_wgsl(&label, &source)?;
        let pipeline = build_source_pipeline(
            &self.device,
            &self.source_pipeline_layout,
            &source,
        ...
        self.pipeline_generation = self.pipeline_generation.saturating_add(1);
        self.successful_reloads = self.successful_reloads.saturating_add(1);
        self.last_reload_reason = format!("editor compile: {label}");
        self.last_reload_error.clear();
        self.last_reload_seconds = self.started.elapsed().as_secs_f64();
        self.last_error.clear();
        Ok(())
    ...
    fn shader_time_seconds(&self) -> f32 {
        let end = self.paused_at.unwrap_or_else(Instant::now);
        end.saturating_duration_since(self.time_origin)
    ...
    fn export_still(&mut self, width: u32, height: u32, path: PathBuf) -> Result<String, String> {
        if width == 0 || height == 0 {
            return Err("export dimensions must be greater than zero".into());
        }
        let limit = self.device.limits().max_texture_dimension_2d;
        if width > limit || height > limit {
            return Err(format!(
                "requested {width}x{height} export exceeds this device's {limit}px texture limit"
            ));
        }

        let texture = self.device.create_texture(&wgpu::TextureDescriptor {
            label: Some("WGSL playground still-export texture"),
            size: wgpu::Extent3d {
                width,
                height,
        ...
        let mut uniforms = self.source_uniforms;
        uniforms.timing[0] = self.shader_time_seconds();
        uniforms.resolution = [width as f32, height as f32, 0.0, 0.0];
        let uniform_buffer = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("WGSL playground still-export uniforms"),
            contents: bytemuck::bytes_of(&uniforms),
            usage: wgpu::BufferUsages::UNIFORM,
        });
        let bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("WGSL playground still-export bind group"),
        ...
        let readback = self.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("WGSL playground still-export readback"),
            size: buffer_size,
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("WGSL playground still-export encoder"),
        });
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("WGSL playground still-export pass"),
        ...
        self.device
            ...
            .map_err(|error| format!("GPU wait failed during PNG export: {error}"))?;
        map_rx
            .recv_timeout(Duration::from_secs(60))
            .map_err(|_| "GPU readback timed out during PNG export".to_string())??;

        let mapped = readback.slice(..).get_mapped_range();
        let mut packed = vec![0u8; unpadded_bytes_per_row as usize * height as usize];
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        self.source_uniforms.timing[0] = self.shader_time_seconds();
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("WGSL playground frame encoder"),
        });
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("shader parameter authoritative render pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
        ...
        while alive.load(Ordering::Relaxed) {
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(RenderCommand::SelectShader { shader, reply }) => {
                        let result = self.activate_shader(shader, "playground preset selection");
                        if let Err(error) = &result {
                            self.mark_reload_failure("preset selection".into(), error.clone());
                        }
                        let _ = reply.send(result);
                    }
                    Ok(RenderCommand::CompileSource { source, label, reply }) => {
                        let result = self.compile_source(source, label);
                        if let Err(error) = &result {
                            self.mark_reload_failure("editor compile".into(), error.clone());
                        }
                        let _ = reply.send(result);
                    }
                    Ok(RenderCommand::TogglePause { reply }) => {
                        let _ = reply.send(Ok(self.toggle_pause()));
                    }
                    Ok(RenderCommand::ResetTime { reply }) => {
                        self.reset_time();
                        let _ = reply.send(Ok(()));
                    }
                    Ok(RenderCommand::ExportStill { width, height, path, reply }) => {
                        let _ = reply.send(self.export_still(width, height, path));
                    }
...
fn validate_editor_wgsl(label: &str, source: &str) -> Result<(), String> {
    let module = naga::front::wgsl::parse_str(source).map_err(|error| {
        format!("WGSL parse failed in {label}:\n{}", error.emit_to_string(source))
    })?;
    ...
    .validate(&module)
    .map_err(|error| {
        format!("WGSL validation failed in {label}:\n{}", error.emit_to_string(source))
    })?;
    for required in [
        "@vertex",
        "fn vs_main",
        ...
        if !source.contains(required) {
            return Err(format!(
                "WGSL playground contract failed in {label}: missing required text '{required}'"
            ));
        }
    }
    Ok(())
