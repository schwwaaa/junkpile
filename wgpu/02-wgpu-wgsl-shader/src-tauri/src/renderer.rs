...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let uniforms = Uniforms { timing: [0.0; 4], image: [width as f32, height as f32, 0.0, 0.0] };
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        self.uniforms.timing[0] = self.current_time(); self.uniforms.image[0] = self.width as f32; self.uniforms.image[1] = self.height as f32;
                    ...
                    Ok(RenderCommand::SetPaused(paused)) => {
                        ...
                        if !paused {
                            if let Some(paused_time) = self.paused_at.take() {
                                self.started = Instant::now(); self.time_offset = paused_time;
                            }
                        }
                    }
                    Ok(RenderCommand::ResetTime) => { self.started = Instant::now(); self.time_offset = 0.0; self.paused_at = self.paused_at.map(|_| 0.0); },
