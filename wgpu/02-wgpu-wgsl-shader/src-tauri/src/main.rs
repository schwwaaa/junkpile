...
#[tauri::command]
fn reset_time(state: tauri::State<'_, RendererHandle>) {
    state.send(RenderCommand::ResetTime);
}
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu WGSL Shader · Native Surface")
        ...
        .invoke_handler(tauri::generate_handler![get_renderer_info, toggle_renderer_fullscreen, set_paused, reset_time])
