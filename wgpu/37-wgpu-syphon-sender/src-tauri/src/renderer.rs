use crate::{
    ...
    syphon::{SyphonConfig, SyphonOutputSink, SyphonStatus},
...
impl RendererHandle {
    ...
    pub fn configure_syphon(&self, config: SyphonConfig) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::ConfigureSyphon { config, reply })?;
        response
            .recv_timeout(Duration::from_secs(5))
            .map_err(|_| "renderer did not answer the Syphon configuration request".to_string())?
    }

    pub fn start_syphon(&self) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::StartSyphon { reply })?;
        response
            .recv_timeout(Duration::from_secs(10))
            .map_err(|_| "renderer did not answer the Syphon start request".to_string())?
    }

    pub fn stop_syphon(&self) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::StopSyphon { reply })?;
        response
            .recv_timeout(Duration::from_secs(5))
            .map_err(|_| "renderer did not answer the Syphon stop request".to_string())?
    }
}
...
pub fn start(window: tauri::Window) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-syphon-sender".into())
...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile Syphon sender device"),
        ...
        let default_config = SyphonConfig::default();
        ...
        let source_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Syphon source shader"),
        ...
        let source_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Syphon source bind group layout"),
        ...
        let source_uniform_buffer =
            device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Syphon source uniforms"),
        ...
        let source_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Syphon source bind group"),
        ...
        let syphon = SyphonOutputSink::new(&device, default_config.clone());
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            syphon: self.syphon.status(),
            contract_summary: "The native renderer produces one authoritative wgpu texture. A bounded three-slot GPU readback path converts RGBA into progressive BGRA frames and a dedicated Syphon worker publishes them under a configurable source name without blocking preview rendering.".into(),
        }
    }

    fn configure_syphon(&mut self, config: SyphonConfig) -> Result<(), String> {
        config.validate(self.max_texture_dimension_2d)?;
        if self.syphon.is_active() {
            return Err("stop Syphon before changing source settings".into());
        }
        self.syphon.reconfigure(&self.device, config.clone())?;
        let (texture, view) =
        ...
        self.frame_descriptor = self.syphon.descriptor();
    ...
    fn render(&mut self) -> Result<(), String> {
        // Syphon is an offscreen output and must continue when the preview window is
        // minimized, occluded, or temporarily unavailable.
        self.syphon.poll_completed(&self.device);
        ...
        let surface_output = if self.minimized {
            ...
            match self.surface.get_current_texture() {
                ...
                wgpu::CurrentSurfaceTexture::Validation => {
                    self.last_error = "surface validation error; Syphon output continues offscreen".into();
                    None
                }
            }
        };
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("Syphon sender frame encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Syphon authoritative render pass"),
        ...
        self.syphon.submit(&video_frame, &mut encoder);
        ...
        while alive.load(Ordering::Relaxed) {
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(RenderCommand::ConfigureSyphon { config, reply }) => {
                        let _ = reply.send(self.configure_syphon(config));
                    }
                    Ok(RenderCommand::StartSyphon { reply }) => {
                        let _ = reply.send(self.syphon.start());
                    }
                    Ok(RenderCommand::StopSyphon { reply }) => {
                        self.syphon.stop();
                        let _ = reply.send(Ok(()));
                    }
                    Ok(RenderCommand::ResetMetrics) => self.syphon.reset_metrics(),
...
fn create_authoritative_target(
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("Syphon authoritative texture"),
        size: wgpu::Extent3d {
            width,
            height,
