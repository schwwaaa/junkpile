...
pub fn start(window: tauri::Window) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-io-profile-router".into())
    ...
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile I/O profile router device"),
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            contract_summary: format!(
                "The armed I/O profile prepares a native {} × {} authoritative texture, GPU preview route, and optional bounded FFmpeg file route. Profile changes reconfigure the route only while the recorder is idle.",
