...
pub fn start(window: tauri::Window) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-high-resolution-record".into())
...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile high-resolution record device"),
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            contract_summary: format!(
                "The authoritative shader renders natively at {} × {}. The same-resolution recording texture is copied through a two-slot mapped-buffer ring and a two-frame bounded FFmpeg queue. High-resolution pressure is measured rather than hidden.",
    ...
    fn reconfigure_authoritative(&mut self, width: u32, height: u32) -> Result<(), String> {
        ...
        self.source_descriptor = FrameDescriptor::rgba_srgb(width, height, TARGET_FPS);
...
fn create_authoritative_target(
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("high-resolution authoritative source texture"),
        size: wgpu::Extent3d {
            width,
            height,
