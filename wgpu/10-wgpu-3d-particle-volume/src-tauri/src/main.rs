...
#[tauri::command]
fn set_depth_occlusion(state: tauri::State<'_, RendererHandle>, enabled: bool) {
    state.send(RenderCommand::SetDepthOcclusion(enabled));
}
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu 3D Particle Volume · Native Surface")
        ...
        .expect("error running wgpu 3D particle volume");
}
