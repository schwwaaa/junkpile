...
pub fn start(window: tauri::Window, recording_directory: PathBuf) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-appliance-runtime".into())
...
impl Renderer {
    async fn new(window: tauri::Window, recording_directory: PathBuf) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile appliance runtime device"),
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            contract_summary: format!(
                "One native {} × {} authoritative texture feeds optional preview, bounded FFmpeg recording, bounded NDI output, and the platform texture-sharing sink (Syphon on macOS or Spout on Windows). Both windows can remain hidden while offscreen rendering and external outputs continue.",
    ...
    fn configure_ndi(&mut self, config: NdiConfig) -> Result<(), String> {
        ...
        if config.width != self.source_descriptor.width || config.height != self.source_descriptor.height {
            return Err(format!(
                "NDI route {} × {} must match the authoritative frame {} × {} in this appliance runtime",
                config.width,
                config.height,
                self.source_descriptor.width,
                self.source_descriptor.height,
    ...
    fn configure_platform_share(&mut self, config: PlatformShareConfig) -> Result<(), String> {
        ...
        if config.width != self.source_descriptor.width || config.height != self.source_descriptor.height {
            return Err(format!(
                "platform-share route {} × {} must match the authoritative frame {} × {} in this appliance runtime",
                config.width,
                config.height,
                self.source_descriptor.width,
                self.source_descriptor.height,
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("appliance runtime frame encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("appliance authoritative source pass"),
