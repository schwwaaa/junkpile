...
fn set_resolution_preset(
    ...
    if !matches!(preset.as_str(), "window" | "1080p" | "4k" | "8k") {
        return Err("preset must be window, 1080p, 4k, or 8k".into());
    }
    state.send(RenderCommand::SetPreset(preset));
    Ok(())
}
...
fn set_param(
    ...
    if !matches!(
        name.as_str(),
        "render_scale" | "complexity" | "speed" | "exposure" | "frequency"
    ) {
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu High Resolution Lab · Native Surface")
        ...
        .expect("error running high resolution lab");
}
