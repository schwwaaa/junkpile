use crate::osc::{OscSnapshot, PARAMETER_COUNT, SIGNAL_COUNT};
...
pub fn start(
    window: tauri::Window,
    osc_snapshot: Arc<RwLock<OscSnapshot>>,
) -> Result<RendererHandle, String> {
    let (tx, rx) = sync_channel(128);
    let alive = Arc::new(AtomicBool::new(true));
    let mut renderer = pollster::block_on(Renderer::new(window, osc_snapshot))?;
    let info = Arc::new(RwLock::new(renderer.info()));

    let thread_info = Arc::clone(&info);
    let thread_alive = Arc::clone(&alive);
    thread::Builder::new()
        .name("junkpile-wgpu-osc-network".into())
...
struct Renderer {
    ...
    osc_buffer: wgpu::Buffer,
    bind_group: wgpu::BindGroup,
    uniforms: Uniforms,
    osc_snapshot: Arc<RwLock<OscSnapshot>>,
    smoothed_parameters: [f32; PARAMETER_COUNT],
    signal_levels: [f32; SIGNAL_COUNT],
    pulse: f32,
...
impl Renderer {
    async fn new(
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile OSC registry device"),
        ...
        let uniforms = Uniforms {
            ...
            osc: [snapshot.last_value, snapshot.last_address_phase, 0.0, 0.0],
        };
        let osc_data = OscGpuData {
            signals: [0.0; SIGNAL_COUNT],
        };

        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("OSC registry uniform buffer"),
            contents: bytemuck::bytes_of(&uniforms),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        let osc_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("OSC signal storage buffer"),
            contents: bytemuck::bytes_of(&osc_data),
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        });

        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("OSC registry bind group layout"),
            entries: &[
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("OSC registry bind group"),
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: osc_buffer.as_entire_binding(),
                },
            ],
        });

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("OSC registry WGSL"),
            source: wgpu::ShaderSource::Wgsl(include_str!("osc.wgsl").into()),
        });
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("OSC registry pipeline layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("OSC registry pipeline"),
        ...
        Ok(Self {
            ...
            signal_levels: [0.0; SIGNAL_COUNT],
            pulse: 0.0,
            last_signal_sequence: snapshot.signal_sequence,
            osc_sequence: snapshot.sequence,
            adapter_info,
            width,
            height,
            minimized: false,
    ...
    fn info(&self) -> RendererInfo {
        RendererInfo {
            ...
            active_signals: self.signal_levels.iter().filter(|value| **value > 0.015).count() as u32,
    ...
    fn update_osc_state(&mut self, delta_seconds: f32) {
        let snapshot = self
            .osc_snapshot
            .read()
            .expect("OSC snapshot poisoned")
            .clone();
        self.osc_sequence = snapshot.sequence;
        ...
        let signal_retention = (0.74 + self.smoothed_parameters[5] * 0.255)
            .clamp(0.74, 0.995)
            .powf(frame_scale);
        for value in &mut self.signal_levels {
            *value *= signal_retention;
            if *value < 0.0001 {
                *value = 0.0;
            }
        }

        if snapshot.signal_sequence != self.last_signal_sequence {
            self.last_signal_sequence = snapshot.signal_sequence;
            for index in 0..SIGNAL_COUNT {
                self.signal_levels[index] = self.signal_levels[index].max(snapshot.signals[index]);
            }
        ...
        self.uniforms.osc = [
            snapshot.last_value,
            snapshot.last_address_phase,
            self.signal_levels.iter().copied().fold(0.0_f32, f32::max),
            self.pulse,
        ];
    }
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        self.update_osc_state(delta as f32);
        ...
        let osc_data = OscGpuData {
            signals: self.signal_levels,
        };
        self.queue
            .write_buffer(&self.uniform_buffer, 0, bytemuck::bytes_of(&self.uniforms));
        self.queue
            .write_buffer(&self.osc_buffer, 0, bytemuck::bytes_of(&osc_data));
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("OSC registry command encoder"),
            });
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("OSC registry render pass"),
