...
fn set_param(state: tauri::State<'_, RendererHandle>, name: String, value: f32) -> Result<(), String> {
    const VALID: &[&str] = &["zoom", "rotation", "exposure", "gamma", "saturation"];
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu Image Texture · Native Surface")
        ...
        .invoke_handler(tauri::generate_handler![get_renderer_info, set_param, set_filter_mode, set_fit_mode, reset_params, toggle_renderer_fullscreen])
        .run(tauri::generate_context!())
        .expect("error while running image-texture example");
}
