...
impl IoProfile {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.ndi.name.trim().is_empty() {
            return Err(format!("profile '{}' ndi.name cannot be empty", self.id));
        }
        if self.ndi.fps_n == 0 || self.ndi.fps_d == 0 {
            return Err(format!(
                "profile '{}' NDI frame-rate numerator and denominator must be greater than zero",
                self.id
            ));
        }
        let ndi_fps = self.ndi.fps_n as f64 / self.ndi.fps_d as f64;
        if !(1.0..=120.0).contains(&ndi_fps) {
            return Err(format!(
                "profile '{}' NDI frame rate must be between 1 and 120 fps",
                self.id
            ));
        }
        Ok(())
