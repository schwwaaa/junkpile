...
fn prepare_route(
    ...
    if matches!(snapshot.recording.state, RecordingState::Recording | RecordingState::Finalizing) || snapshot.ndi.active {
    ...
    window
        .set_visible(profile.preview.enabled)
        .map_err(|error| format!("could not update preview visibility: {error}"))?;
    Ok(())
}
...
fn start_profile_outputs(
    ...
    if !profile.recording.enabled && !profile.ndi.enabled {
        return Err(format!(
...
fn show_renderer_window(app: tauri::AppHandle) -> Result<(), String> {
    let window = app
        .get_window("renderer")
        .ok_or_else(|| "renderer window is not available".to_string())?;
    window.set_visible(true).map_err(|error| error.to_string())?;
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile 36 · Runtime Output Router · Native Surface")
        ...
        .expect("error while running Junkpile runtime output router example");
}
