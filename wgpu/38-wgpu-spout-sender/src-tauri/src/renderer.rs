use crate::{
    ...
    spout::{SpoutConfig, SpoutOutputSink, SpoutStatus},
...
impl RendererHandle {
    ...
    pub fn configure_spout(&self, config: SpoutConfig) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::ConfigureSpout { config, reply })?;
        response
            .recv_timeout(Duration::from_secs(5))
            .map_err(|_| "renderer did not answer the Spout configuration request".to_string())?
    }

    pub fn start_spout(&self) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::StartSpout { reply })?;
        response
            .recv_timeout(Duration::from_secs(10))
            .map_err(|_| "renderer did not answer the Spout start request".to_string())?
    }

    pub fn stop_spout(&self) -> Result<(), String> {
        let (reply, response) = sync_channel(1);
        self.send(RenderCommand::StopSpout { reply })?;
        response
            .recv_timeout(Duration::from_secs(5))
            .map_err(|_| "renderer did not answer the Spout stop request".to_string())?
    }
}
...
pub fn start(window: tauri::Window) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-spout-sender".into())
...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile Spout sender device"),
        ...
        let default_config = SpoutConfig::default();
        ...
        let source_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Spout source shader"),
        ...
        let source_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Spout source bind group layout"),
        ...
        let source_uniform_buffer =
            device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Spout source uniforms"),
        ...
        let source_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Spout source bind group"),
        ...
        let spout = SpoutOutputSink::new(&device, default_config.clone());
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            spout: self.spout.status(),
            contract_summary: "The native renderer produces one authoritative wgpu texture. A bounded three-slot GPU readback path converts RGBA into progressive BGRA frames and a dedicated Spout worker publishes them under a configurable source name without blocking preview rendering.".into(),
        }
    }

    fn configure_spout(&mut self, config: SpoutConfig) -> Result<(), String> {
        config.validate(self.max_texture_dimension_2d)?;
        if self.spout.is_active() {
            return Err("stop Spout before changing source settings".into());
        }
        self.spout.reconfigure(&self.device, config.clone())?;
        let (texture, view) =
        ...
        self.frame_descriptor = self.spout.descriptor();
    ...
    fn render(&mut self) -> Result<(), String> {
        // Spout is an offscreen output and must continue when the preview window is
        // minimized, occluded, or temporarily unavailable.
        self.spout.poll_completed(&self.device);
        ...
        let surface_output = if self.minimized {
            ...
            match self.surface.get_current_texture() {
                ...
                wgpu::CurrentSurfaceTexture::Validation => {
                    self.last_error = "surface validation error; Spout output continues offscreen".into();
                    None
                }
            }
        };
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("Spout sender frame encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Spout authoritative render pass"),
        ...
        self.spout.submit(&video_frame, &mut encoder);
        ...
        while alive.load(Ordering::Relaxed) {
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(RenderCommand::ConfigureSpout { config, reply }) => {
                        let _ = reply.send(self.configure_spout(config));
                    }
                    Ok(RenderCommand::StartSpout { reply }) => {
                        let _ = reply.send(self.spout.start());
                    }
                    Ok(RenderCommand::StopSpout { reply }) => {
                        self.spout.stop();
                        let _ = reply.send(Ok(()));
                    }
                    Ok(RenderCommand::ResetMetrics) => self.spout.reset_metrics(),
...
fn create_authoritative_target(
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("Spout authoritative texture"),
        size: wgpu::Extent3d {
            width,
            height,
