...
fn start_spout(
    ...
    state.configure_spout(config)?;
    state.start_spout()
}

#[tauri::command]
fn stop_spout(state: tauri::State<'_, RendererHandle>) -> Result<(), String> {
    state.stop_spout()
}
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile 38 · Spout Sender · Native Surface")
        ...
        .expect("error while running Junkpile Spout sender example");
}
