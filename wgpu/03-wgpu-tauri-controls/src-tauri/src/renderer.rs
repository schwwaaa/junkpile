...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("native shader pipeline layout"),
        ...
        let uniforms = Uniforms { timing: [0.0, 1.0, 0.0, 0.0], image: [width as f32, height as f32, 1.0, 0.45], look: [205.0, 1.0, 0.82, 0.75], extras: [5.0, 0.0, 0.0, 0.0] };
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        self.uniforms.timing[0] = self.current_time(); self.uniforms.timing[3] = self.frame_count as f32; self.uniforms.image[0] = self.width as f32; self.uniforms.image[1] = self.height as f32;
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(RenderCommand::SetParam(name, value)) => match name.as_str() {
                        "speed" => self.uniforms.timing[1] = value.clamp(0.0, 4.0),
                        "zoom" => self.uniforms.image[2] = value.clamp(0.2, 6.0),
                        "distortion" => self.uniforms.image[3] = value.clamp(0.0, 1.0),
                        "hue" => self.uniforms.look[0] = value.rem_euclid(360.0),
                        "brightness" => self.uniforms.look[1] = value.clamp(0.0, 2.5),
                        "saturation" => self.uniforms.look[2] = value.clamp(0.0, 1.0),
                        "glow" => self.uniforms.look[3] = value.clamp(0.0, 2.0),
                        "complexity" => self.uniforms.extras[0] = value.clamp(1.0, 8.0).round(),
                        "pulse" => self.uniforms.extras[1] = if value >= 0.5 { 1.0 } else { 0.0 },
                        _ => {}
                    },
                    Ok(RenderCommand::ResetParams) => {
                        self.uniforms.timing[1] = 1.0; self.uniforms.image[2] = 1.0; self.uniforms.image[3] = 0.45;
                        self.uniforms.look = [205.0, 1.0, 0.82, 0.75]; self.uniforms.extras[0] = 5.0; self.uniforms.extras[1] = 0.0;
                    }
                    Ok(RenderCommand::SetPaused(paused)) => {
                        if paused && self.paused_at.is_none() { self.paused_at = Some(self.current_time()); }
                        if !paused { if let Some(t) = self.paused_at.take() { self.started = Instant::now(); self.time_offset = t; } }
                    },
