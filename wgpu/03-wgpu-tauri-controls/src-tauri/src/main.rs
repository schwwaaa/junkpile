...
fn set_param(state: tauri::State<'_, RendererHandle>, name: String, value: f32) -> Result<(), String> {
    const VALID: &[&str] = &["speed", "zoom", "distortion", "hue", "brightness", "saturation", "glow", "complexity", "pulse"];
    if !VALID.contains(&name.as_str()) { return Err(format!("unknown renderer parameter: {name}")); }
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu Tauri Controls · Native Surface")
        ...
        .invoke_handler(tauri::generate_handler![get_renderer_info, toggle_renderer_fullscreen, set_param, reset_params, set_paused])
