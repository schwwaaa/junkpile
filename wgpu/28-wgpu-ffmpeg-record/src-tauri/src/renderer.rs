...
const DEFAULT_RECORD_WIDTH: u32 = 960;
const DEFAULT_RECORD_HEIGHT: u32 = 540;
...
pub fn start(window: tauri::Window) -> Result<RendererHandle, String> {
    ...
    thread::Builder::new()
        .name("junkpile-wgpu-ffmpeg-record".into())
...
impl Renderer {
    async fn new(window: tauri::Window) -> Result<Self, String> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("junkpile ffmpeg record device"),
        ...
        let authoritative_texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("authoritative recording source texture"),
            size: wgpu::Extent3d {
                width: FRAME_WIDTH,
                height: FRAME_HEIGHT,
    ...
    fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            ...
            contract_summary: "The 1280 × 720 authoritative GPU frame is scaled into an independent recording texture, copied through a three-slot mapped-buffer ring, packed into RGBA CPU frames, and delivered to a bounded FFmpeg worker without blocking presentation.".into(),
        }
    }
    ...
    fn render(&mut self) -> Result<(), String> {
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("ffmpeg recording frame encoder"),
            });

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("authoritative recording source pass"),
