...
fn sample_video(uv: vec2<f32>) -> vec3<f32> {
    return textureSample(video_tex, video_sampler, clamp(uv, vec2<f32>(0.0), vec2<f32>(1.0))).rgb;
}

fn edge_value(uv: vec2<f32>) -> f32 {
    let texel = 1.0 / max(u.viewport_source.zw, vec2<f32>(1.0));
    let left = luminance(sample_video(uv - vec2<f32>(texel.x, 0.0)));
    let right = luminance(sample_video(uv + vec2<f32>(texel.x, 0.0)));
    let up = luminance(sample_video(uv - vec2<f32>(0.0, texel.y)));
    let down = luminance(sample_video(uv + vec2<f32>(0.0, texel.y)));
    let gx = right - left;
    let gy = down - up;
...
fn fs_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    if (mode == 1) {
        ...
        color = sample_video(warped_uv);
    } else if (mode == 3) {
        // Express channel separation in source pixels so the effect remains
        // predictable at 480p, 1080p, 4K, and other source definitions.
        let source_texel = 1.0 / max(u.viewport_source.zw, vec2<f32>(1.0));
        let separation_pixels = u.effects.y * strength;
        let separation = source_texel * vec2<f32>(separation_pixels, separation_pixels * 0.18);
        let red_sample = sample_video(uv + separation);
        let green_sample = sample_video(uv);
        let blue_sample = sample_video(uv - separation);
        color = vec3<f32>(red_sample.r, green_sample.g, blue_sample.b);
    } else if (mode == 4) {
