...
fn set_toggle(
    ...
    if !matches!(
        name.as_str(),
        "paused" | "auto_impulse" | "backface_culling"
    ) {
...
#[tauri::command]
fn trigger_impulse(state: tauri::State<'_, RendererHandle>) {
    state.send(RenderCommand::TriggerImpulse);
}

#[tauri::command]
fn reset_simulation(state: tauri::State<'_, RendererHandle>) {
    state.send(RenderCommand::ResetSimulation);
}
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile 24 · Mesh Feedback Deformer · Native Surface")
        ...
        .expect("error while running Junkpile 24 mesh feedback deformer");
}
