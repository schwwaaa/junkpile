...
impl SyphonConfig {
    pub fn validate(&self, max_texture_dimension_2d: u32) -> Result<(), String> {
        if self.name.trim().is_empty() {
            return Err("Syphon source name cannot be empty".into());
        }
        if self.width < 16 || self.height < 16 {
            return Err("Syphon dimensions must be at least 16 × 16".into());
        }
        if self.width % 2 != 0 {
            return Err("Syphon frame width must be divisible by two".into());
        }
        ...
        if self.fps_n == 0 || self.fps_d == 0 {
            return Err("Syphon frame-rate numerator and denominator must be greater than zero".into());
        }
        let fps = self.fps();
        if !(1.0..=120.0).contains(&fps) {
            return Err("Syphon frame rate must be between 1 and 120 fps".into());
        }
        Ok(())
    }
...
impl WorkerSignals {
    fn new() -> Self {
        Self {
            ...
            skipped_no_clients: AtomicU64::new(0),
...
struct SyphonWorker {
    tx: Option<SyncSender<SyphonFramePacket>>,
...
impl SyphonWorker {
    fn spawn(
        ...
        #[cfg(feature = "syphon")]
        {
            let (tx, rx) = sync_channel::<SyphonFramePacket>(WORKER_QUEUE_CAPACITY);
            ...
            let thread = thread::Builder::new()
                .name("junkpile-syphon-sender".into())
                .spawn(move || {
                    run_syphon_worker(
                        rx,
                        config,
                ...
                .map_err(|error| format!("could not start Syphon worker thread: {error}"))?;
            startup_rx
                .recv_timeout(Duration::from_secs(8))
                .map_err(|_| "Syphon worker did not finish runtime initialization".to_string())??;
            Ok(Self {
                tx: Some(tx),
                signals,
                thread: Some(thread),
        ...
        #[cfg(not(feature = "syphon"))]
        {
            ...
            Err("this build does not include Syphon; run `npm run dev` or build with Cargo feature `syphon`".into())
        }
    }

    fn try_send(&self, packet: SyphonFramePacket) -> Result<(), TrySendError<SyphonFramePacket>> {
...
fn run_syphon_worker(
    rx: Receiver<SyphonFramePacket>,
    config: SyphonConfig,
    recycle_tx: SyncSender<Vec<u8>>,
    available_buffers: Arc<AtomicU64>,
    signals: Arc<WorkerSignals>,
    ...
    extern "C" {
        fn junkpile_syphon_server_create(name_utf8: *const c_char) -> *mut c_void;
        fn junkpile_syphon_server_destroy(server_ptr: *mut c_void);
        fn junkpile_syphon_server_has_clients(server_ptr: *mut c_void) -> c_int;
        fn junkpile_syphon_server_publish_bgra(
    ...
    let name = match CString::new(config.name.trim()) {
        Ok(value) => value,
        Err(_) => {
            let message = "Syphon source name contains an embedded NUL byte".to_string();
    ...
    let server = unsafe { junkpile_syphon_server_create(name.as_ptr()) };
    if server.is_null() {
        let message = "failed to create SyphonMetalServer; verify the bundled Syphon framework and Metal availability".to_string();
        signals.set_error(&message);
        signals.finished.store(true, Ordering::Relaxed);
        let _ = startup_tx.send(Err(message));
        return;
    ...
    signals.set_log(format!(
        "Syphon source '{}' started at {} × {} · {}/{} fps · BGRA Metal upload",
        config.name, config.width, config.height, config.fps_n, config.fps_d
    ));
    let _ = startup_tx.send(Ok(()));

    loop {
        let has_clients = unsafe { junkpile_syphon_server_has_clients(server) != 0 };
        signals.has_clients.store(has_clients, Ordering::Relaxed);
        ...
        if !config.only_when_clients || has_clients {
            published = unsafe {
                junkpile_syphon_server_publish_bgra(
                    server,
                    packet.bgra.as_ptr(),
                    config.width,
                    config.height,
            ...
            if !published {
                signals.set_error("Syphon Metal texture upload or publish failed");
            }
        } else {
            signals.skipped_no_clients.fetch_add(1, Ordering::Relaxed);
        }
        ...
        if published {
            ...
            signals
                .raw_bytes
                .fetch_add(packet.bgra.len() as u64, Ordering::Relaxed);
    ...
    unsafe { junkpile_syphon_server_destroy(server) };
    signals.has_clients.store(false, Ordering::Relaxed);
    signals.running.store(false, Ordering::Relaxed);
    signals.finished.store(true, Ordering::Relaxed);
    signals.set_log(format!("Syphon source '{}' stopped", config.name));
}

#[cfg(all(feature = "syphon", not(target_os = "macos")))]
fn run_syphon_worker(
    _rx: Receiver<SyphonFramePacket>,
    _config: SyphonConfig,
    _recycle_tx: SyncSender<Vec<u8>>,
    _available_buffers: Arc<AtomicU64>,
    signals: Arc<WorkerSignals>,
    startup_tx: SyncSender<Result<(), String>>,
) {
    let message = "Syphon output is available only on macOS".to_string();
    signals.set_error(&message);
    signals.finished.store(true, Ordering::Relaxed);
    let _ = startup_tx.send(Err(message));
}

pub struct SyphonOutputSink {
    config: SyphonConfig,
...
impl SyphonOutputSink {
    pub fn new(device: &wgpu::Device, config: SyphonConfig) -> Self {
        ...
        for _ in 0..CPU_BUFFER_POOL_COUNT {
            recycle_tx
                ...
                .expect("could not initialize Syphon CPU frame buffers");
        }
        Self {
            config,
            descriptor,
    ...
    pub fn config(&self) -> SyphonConfig {
        self.config.clone()
    }
    ...
    pub fn reconfigure(&mut self, device: &wgpu::Device, config: SyphonConfig) -> Result<(), String> {
        if self.active {
            return Err("stop the Syphon sender before changing its configuration".into());
        }
        if self
            .readback_slots
            .iter()
        ...
        {
            return Err("wait for the final GPU readback before changing Syphon configuration".into());
        }
        self.worker.take();
        self.config = config;
        ...
        for _ in 0..CPU_BUFFER_POOL_COUNT {
            recycle_tx
                ...
                .map_err(|_| "could not recreate Syphon CPU buffer pool".to_string())?;
        }
    ...
    pub fn start(&mut self) -> Result<(), String> {
        if self.active {
            return Err("Syphon sender is already active".into());
        }
        let worker = SyphonWorker::spawn(
            self.config.clone(),
    ...
    pub fn reset_metrics(&mut self) {
        ...
        if let Some(worker) = &self.worker {
            ...
            worker.signals.skipped_no_clients.store(0, Ordering::Relaxed);
        }
    }
        ...
        loop {
            match self.map_rx.try_recv() {
                ...
                Err(TryRecvError::Disconnected) => {
                    self.last_error = "Syphon GPU map completion channel disconnected".into();
                    break;
                }
            }
        }
    ...
    pub fn submit(&mut self, frame: &VideoFrame<'_>, encoder: &mut wgpu::CommandEncoder) {
        ...
        if self.config.only_when_clients
            && self.worker.as_ref().is_some_and(|worker| {
                !worker.signals.has_clients.load(Ordering::Relaxed)
            })
        {
            if let Some(worker) = &self.worker {
                worker
    ...
    pub fn status(&self) -> SyphonStatus {
        ...
        if let Some(worker) = &self.worker {
            ...
            has_clients = worker.signals.has_clients.load(Ordering::Relaxed);
            skipped_no_clients = worker.signals.skipped_no_clients.load(Ordering::Relaxed);
        ...
        let state = if !last_error.is_empty() {
            "error"
        } else if self.active && worker_running && self.config.only_when_clients && !has_clients {
            "waiting"
        ...
        SyphonStatus {
            feature_enabled: cfg!(all(feature = "syphon", target_os = "macos")),
            active: self.active,
            state: state.into(),
    ...
    fn handle_map_completion(&mut self, completion: MapCompletion) {
        let Some(slot) = self.readback_slots.get_mut(completion.slot_index) else {
            self.last_error = format!("invalid Syphon readback slot {}", completion.slot_index);
            return;
        };
        let metadata = match &slot.state {
            ...
            ReadbackState::Idle => {
                self.last_error = format!(
                    "Syphon readback slot {} completed without a pending map",
        ...
        if let Err(error) = completion.result {
            ...
            self.dropped_gpu += 1;
            self.last_error = format!("Syphon GPU readback failed: {error}");
            return;
        }
        ...
        let Some(worker) = self.worker.as_ref() else {
            ...
            self.last_error = "Syphon worker is unavailable".into();
            return;
        };
        match worker.try_send(packet) {
            ...
            Err(TrySendError::Disconnected(packet)) => {
                self.dropped_worker += 1;
                self.recycle_buffer(packet.bgra);
                self.last_error = "Syphon worker channel disconnected".into();
                self.active = false;
            }
        }
    }
...
impl Drop for SyphonOutputSink {
    fn drop(&mut self) {
        self.stop();
    }
}
    ...
    let slots = (0..READBACK_SLOT_COUNT)
        .map(|index| ReadbackSlot {
            buffer: Arc::new(device.create_buffer(&wgpu::BufferDescriptor {
                label: Some(&format!("Syphon readback slot {index}")),
                size: buffer_size,
