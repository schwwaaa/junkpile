...
#[tauri::command]
fn set_renderer_size(app: tauri::AppHandle, width: u32, height: u32) -> Result<(), String> {
    let window = app.get_window("renderer").ok_or_else(|| "renderer window is not available".to_string())?;
    window
    ...
    tauri::Builder::default()
        .setup(|app| {
            let renderer_window = tauri::window::WindowBuilder::new(app, "renderer")
                .title("Junkpile wgpu Resize + Fullscreen · Native Surface")
        ...
        .invoke_handler(tauri::generate_handler![get_renderer_info, toggle_renderer_fullscreen, set_renderer_size])
