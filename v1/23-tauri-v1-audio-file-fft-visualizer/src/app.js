...
// =============================================================================
// Junkpile 23 — Tauri v1 Audio File FFT Visualizer
//
// Signal path:
...
async function saveBlob(blob, suggestedName, extension) {
  ...
  if (tauriApi?.dialog?.save && tauriApi?.fs?.writeBinaryFile) {
    const path = await tauriApi.dialog.save({ defaultPath: suggestedName, filters: [{ name: extension.toUpperCase(), extensions: [extension] }] });
    if (!path) return null;
...
async function saveLastRecording() {
  ...
  try { const extension = lastRecordingMime.includes('mp4') ? 'mp4' : 'webm'; const path = await saveBlob(lastRecordingBlob, `${safeBaseName(currentFile?.name)}-visual.${extension}`, extension); if (path) $('last-save').textContent = path.split(/[\\/]/).pop(); } catch (error) { console.error('Save failed', error); setRecordState('Save failed', 'error'); }
}
async function saveSnapshot() {
  try { const blob = await new Promise((resolve, reject) => canvas.toBlob((value) => value ? resolve(value) : reject(new Error('PNG encoding failed')), 'image/png')); const path = await saveBlob(blob, `${safeBaseName(currentFile?.name)}-frame.png`, 'png'); if (path) $('last-save').textContent = path.split(/[\\/]/).pop(); } catch (error) { console.error('Snapshot failed', error); setRecordState('PNG failed', 'error'); }
}
