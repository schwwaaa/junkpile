...
const FRAG_SHADER = `
    ...
    // fi is declared before the for loop — WebKit's GLSL compiler rejects
    // variable declarations inside a for-loop body when an int counter (i)
    // is already in scope. Hoisting fi fixes the WKWebView/Tauri parse error.
    float fbm(vec2 p) {
        float value     = 0.0;
        float amplitude = 0.5;
        float freq      = 1.0;
