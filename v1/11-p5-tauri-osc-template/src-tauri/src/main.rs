...
fn emit_message(message: OscMessage, app: &AppHandle, sender: SocketAddr, bundle_depth: u32) {
    ...
    if let Err(error) = app.emit_all("osc-event", event) {
...
fn start_osc_listener(
    ...
    let listener_thread = thread::Builder::new()
        .name("junkpile-v1-osc-listener".to_string())
        .spawn(move || {
                ...
                match socket.recv_from(&mut buffer) {
                    Ok((size, sender)) => match rosc::decoder::decode_udp(&buffer[..size]) {
                        ...
                        Err(error) => {
                            ...
                            let _ = thread_app.emit_all("osc-error", message);
                        }
                    },
                    Err(error)
                        if matches!(
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running Junkpile V1 Example 11");
}
