...
const convertFileSrc = tauri.tauri?.convertFileSrc;
...
function clearCache() {
  ...
  state.cache.forEach((entry) => { if (entry.image) { entry.image.onload = null; entry.image.onerror = null; entry.image.src = ""; } });
  state.cache.clear(); state.loadQueue.length = 0; state.activeLoads = 0; state.uploadedA = null; state.uploadedB = null; state.lastReadySource = null;
}
...
function pumpLoads() {
  while (state.activeLoads < state.maxLoads && state.loadQueue.length) {
    ...
    image.onload = () => {
      ...
      if (!state.sourceWidth || state.sourceWidth === 1 || index === 0) { state.sourceWidth = image.naturalWidth || image.width; state.sourceHeight = image.naturalHeight || image.height; }
    ...
    image.onerror = () => {
      if (generation !== state.cacheGeneration || !state.cache.has(index)) return;
      state.activeLoads -= 1;
      entry.status = "error"; entry.error = `Could not decode ${state.paths[index]}`;
      pumpLoads();
    };
    try { image.src = convertFileSrc ? convertFileSrc(state.paths[index]) : state.paths[index]; }
    catch (error) { image.onerror = null; state.activeLoads -= 1; entry.status = "error"; entry.error = error.message || String(error); pumpLoads(); }
  }
}
...
function scheduleCacheAround(frameIndex) {
  ...
  if (state.cache.size > radius * 3 + 12) {
    ...
    [...state.cache.entries()].forEach(([index, entry]) => {
      if (!keep.has(index) && entry.status !== "loading") { if (entry.image) entry.image.src = ""; state.cache.delete(index); }
    });
  }
}
...
async function refreshMonitors() {
  try {
    ...
    const simple = monitors.map((monitor) => ({ name: monitor.name || "Display", width: monitor.size?.width || 0, height: monitor.size?.height || 0, position: monitor.position ? { x: monitor.position.x, y: monitor.position.y } : null, scaleFactor: monitor.scaleFactor || 1 }));
    if (tauriEvent) await tauriEvent.emit("sequence-monitors", simple);
  } catch (error) { console.warn("Monitor enumeration failed", error); if (tauriEvent) await tauriEvent.emit("sequence-monitors", []); }
}
...
async function moveToMonitor(index, fullscreen = false) {
  ...
  if (!monitor || !appWindow) { await emitStatus({ message: "Display placement API unavailable" }); return; }
  try {
    ...
    await emitStatus({ message: fullscreen ? `Fullscreen on display ${index + 1}` : `Moved to display ${index + 1}` });
...
async function toggleFullscreen(index) {
  ...
  try { const fullscreen = await appWindow.isFullscreen(); if (fullscreen) { await appWindow.setFullscreen(false); await emitStatus({ message: "Output windowed" }); } else await moveToMonitor(index, true); }
...
async function snapshot() {
  try {
    const blob = await new Promise((resolve,reject)=>canvas.toBlob((value)=>value?resolve(value):reject(new Error("PNG creation failed")),"image/png")); const name=snapshotName();
    if (tauriDialog?.save && tauriFs?.writeBinaryFile) { const path=await tauriDialog.save({defaultPath:name,filters:[{name:"PNG image",extensions:["png"]}]}); if(!path)return; await tauriFs.writeBinaryFile(path,new Uint8Array(await blob.arrayBuffer())); await emitStatus({message:`Saved ${path}`}); }
    else { const url=URL.createObjectURL(blob); const anchor=document.createElement("a"); anchor.href=url; anchor.download=name; anchor.click(); setTimeout(()=>URL.revokeObjectURL(url),1000); await emitStatus({message:"PNG downloaded"}); }
...
async function initialize() {
  ...
  if (tauriWindow?.appWindow?.onCloseRequested) await tauriWindow.appWindow.onCloseRequested(async (event) => { event.preventDefault(); await tauriWindow.appWindow.hide(); await emitStatus({ message: "Output hidden — use Show output to reopen" }); });
