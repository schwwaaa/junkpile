...
// =============================================================================
// Junkpile 12 — Tauri v1 Video Texture Player
...
async function loadVideoFile(file) {
  ...
  try {
    ...
    sourceSize.textContent = `${video.videoWidth} × ${video.videoHeight} · ${(file.size / (1024 * 1024)).toFixed(1)} MB`;
    timeline.value = '0';
...
for (const target of [document.body, dropZone, stage]) {
  ...
  target.addEventListener('drop', (event) => {
    ...
    const file = files.find((candidate) => candidate.type.startsWith('video/'));
    if (file) loadVideoFile(file);
  });
}
...
function uploadVideoFrame() {
  ...
  if (current === latestUploadedTime) return;
