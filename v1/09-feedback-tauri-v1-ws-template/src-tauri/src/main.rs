...
async fn run_listener(bind_addr: String, clients: ClientMap) -> Result<(), String> {
    ...
    println!("[junkpile 09] listening on ws://{bind_addr}");
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle_ws(stream, peer_addr, clients).await {
                eprintln!("[junkpile 09] client {peer_addr} error: {error}");
            }
        });
    }
}
...
async fn handle_ws(
    ...
    let reader = tokio::spawn(async move {
        while let Some(message) = ws_rx.next().await {
            match message {
                Ok(Message::Text(text)) => {
                    if let Ok(hello) = serde_json::from_str::<HelloMessage>(&text) {
                        if hello.r#type == "hello" && !hello.role.is_empty() {
                            ...
                            println!("[junkpile 09] {peer_addr} identified as role='{}'", hello.role);
                ...
                Ok(Message::Binary(bytes)) => {
                    ...
                    if sent == 0 {
                        println!("[junkpile 09] binary data arrived before a canvas client connected");
                    }
                }
                ...
                Ok(_) => {}
                Err(error) => {
                    eprintln!("[junkpile 09] receive error from {peer_addr}: {error}");
                    break;
                }
            }
        }
        ...
        broadcast_presence(&reader_clients).await;
        println!("[junkpile 09] {peer_addr} disconnected");
    });
...
fn main() {
    tauri::Builder::default()
        .setup(|_app| {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = run_listener(format!("127.0.0.1:{PORT}"), clients_v4).await {
                    eprintln!("[junkpile 09] IPv4 listener error: {error}");
                }
            });
            tauri::async_runtime::spawn(async move {
                if let Err(error) = run_listener(format!("[::1]:{PORT}"), clients_v6).await {
                    eprintln!("[junkpile 09] IPv6 listener error: {error}");
                }
            });
            Ok(())
        })
        ...
        .expect("error while running Junkpile Example 09");
}
