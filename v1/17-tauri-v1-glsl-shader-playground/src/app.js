...
void main(){vec2 uv=v_uv;float wave=.5+.5*sin((uv.x+uv.y)*8.0+u_time);float m=mix(uv.x,wave,u_param0);vec3 c=mix(u_color_a,u_color_b,m);c*=.65+.35*uv.y;gl_FragColor=vec4(c,1.0);}` }
  ...
  const state={gl:null,buffer:null,program:null,attr:-1,uniforms:{},preset:"domain",time:0,last:performance.now(),frames:0,fpsAt:performance.now(),mouse:[.5,.5],compileTimer:0,compileCount:0,resize:true,terminalCount:0};
  ...
  async function saveSource(){const name=`${stem(ui.shaderTitle.textContent)}.frag`,source=ui.shaderEditor.value;try{if(tauriApi?.dialog?.save&&tauriApi?.fs?.writeBinaryFile){const path=await tauriApi.dialog.save({defaultPath:name,filters:[{name:"GLSL shader",extensions:["frag","glsl","fs"]}]});if(!path)return;await tauriApi.fs.writeBinaryFile(path,new TextEncoder().encode(source));ui.fileState.textContent="Saved";return;}}catch(e){console.error(e)}download(new Blob([source],{type:"text/plain"}),name);ui.fileState.textContent="Downloaded";}
  async function snapshot(){ui.snapshot.disabled=true;ui.captureState.textContent="Encoding";try{const blob=await new Promise((res,rej)=>ui.canvas.toBlob(b=>b?res(b):rej(new Error("PNG encoding failed")),"image/png"));const name=`${stem(ui.shaderTitle.textContent)}-${ui.canvas.width}x${ui.canvas.height}.png`;if(tauriApi?.dialog?.save&&tauriApi?.fs?.writeBinaryFile){const path=await tauriApi.dialog.save({defaultPath:name,filters:[{name:"PNG",extensions:["png"]}]});if(!path){ui.captureState.textContent="Cancelled";return;}await tauriApi.fs.writeBinaryFile(path,new Uint8Array(await blob.arrayBuffer()));ui.captureState.textContent="Saved";}else{download(blob,name);ui.captureState.textContent="Downloaded";}}catch(e){console.error(e);ui.captureState.textContent="Failed";}finally{ui.snapshot.disabled=false;setTimeout(()=>ui.captureState.textContent="Ready",1600);}}
  function loadFile(file){if(!file)return;const r=new FileReader();r.onload=()=>{ui.shaderEditor.value=String(r.result||"").replace(/^\uFEFF/,"");ui.shaderTitle.textContent=stem(file.name);ui.hudName.textContent=file.name;ui.fileState.textContent=file.name;lineNumbers();cursor();terminal("info",`Loaded ${file.name} · ${file.size} bytes`);schedule();};r.onerror=()=>terminal("error",`Could not read ${file.name}.`);r.readAsText(file);}
  function drop(e){e.preventDefault();e.stopPropagation();ui.dropZone.classList.remove("dragging");ui.dropOverlay.classList.add("hidden");loadFile(e.dataTransfer?.files?.[0]);}

  function bind(){ui.compile.addEventListener("click",()=>compileCurrent());ui.clearTerminal.addEventListener("click",()=>clearTerminal());ui.autoCompile.addEventListener("change",()=>ui.autoCompile.checked?schedule():status("Manual","busy"));ui.format.addEventListener("click",formatSource);ui.reset.addEventListener("click",()=>loadPreset(state.preset));ui.loadPreset.addEventListener("click",()=>loadPreset(ui.preset.value));ui.saveSource.addEventListener("click",saveSource);ui.copySource.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(ui.shaderEditor.value)}catch{ui.shaderEditor.select();document.execCommand("copy")}ui.fileState.textContent="Copied";});ui.snapshot.addEventListener("click",snapshot);ui.resetTime.addEventListener("click",()=>state.time=0);ui.fileInput.addEventListener("change",()=>{loadFile(ui.fileInput.files?.[0]);ui.fileInput.value=""});ui.shaderEditor.addEventListener("input",()=>{lineNumbers();cursor();ui.fileState.textContent="Modified";schedule();});ui.shaderEditor.addEventListener("paste",event=>{const length=event.clipboardData?.getData("text")?.length||0;setTimeout(()=>terminal("info",`Pasted ${length} characters; ${ui.autoCompile.checked?"automatic compile queued":"press Compile to test"}.`),0);});ui.shaderEditor.addEventListener("scroll",()=>ui.lineNumbers.scrollTop=ui.shaderEditor.scrollTop);["click","keyup"].forEach(ev=>ui.shaderEditor.addEventListener(ev,cursor));ui.shaderEditor.addEventListener("keydown",e=>{if(e.key==="Tab"){e.preventDefault();ui.shaderEditor.setRangeText("  ",ui.shaderEditor.selectionStart,ui.shaderEditor.selectionEnd,"end");lineNumbers();schedule();}else if(e.key==="Enter"&&(e.metaKey||e.ctrlKey)){e.preventDefault();compileCurrent();}});[ui.speed,ui.p0,ui.p1,ui.p2,ui.p3,ui.colorA,ui.colorB].forEach(el=>el.addEventListener("input",output));ui.scale.addEventListener("input",output);[ui.pause,ui.mouseOn].forEach(el=>el.addEventListener("change",output));ui.stage.addEventListener("pointermove",e=>{if(!ui.mouseOn.checked)return;const r=ui.stage.getBoundingClientRect();state.mouse=[Math.min(1,Math.max(0,(e.clientX-r.left)/r.width)),Math.min(1,Math.max(0,1-(e.clientY-r.top)/r.height))];ui.hudMouse.textContent=`Mouse ${state.mouse[0].toFixed(2)} · ${state.mouse[1].toFixed(2)}`;});[ui.dropZone,ui.stage].forEach(el=>{el.addEventListener("dragover",e=>{e.preventDefault();ui.dropZone.classList.add("dragging");ui.dropOverlay.classList.remove("hidden")});el.addEventListener("dragleave",()=>{ui.dropZone.classList.remove("dragging");ui.dropOverlay.classList.add("hidden")});el.addEventListener("drop",drop)});window.addEventListener("resize",resize);window.addEventListener("error",event=>terminal("runtime",`${event.message}${event.lineno?` · line ${event.lineno}`:""}`));window.addEventListener("unhandledrejection",event=>terminal("runtime",`Unhandled promise rejection: ${event.reason?.message||event.reason||"Unknown error"}`));}
  bind();output();loadPreset("domain",false);clearTerminal("Shader terminal ready. This renderer targets WebGL 1 / GLSL ES 1.00.");if(initGl()){terminal("info",`WebGL initialized · ${state.gl.getParameter(state.gl.RENDERER)||"renderer unavailable"}`);compileCurrent(true);requestAnimationFrame(frame);}
})();
