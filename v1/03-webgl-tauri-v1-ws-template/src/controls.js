...
function connect() {
  ...
  socket.addEventListener("message", event => {
    ...
    if (message.type === "presence") {
      ...
      if (message.renderer) setText("rendererReadout", String(message.renderer).slice(0, 26));
      state.paused = Boolean(message.paused);
      updatePauseUi();
      if (message.shaderReady === false) {
        setStatus("canvasStatus", "Shader error", "error");
      } else {
...
async function toggleOutputFullscreen() {
  try {
    ...
    console.error("[junkpile 03] fullscreen failed", error);
  }
}
...
async function showOutput() {
  try {
    ...
    console.error("[junkpile 03] show output failed", error);
  }
}
...
async function focusOutput() {
  try {
    ...
    console.error("[junkpile 03] focus output failed", error);
  }
}
