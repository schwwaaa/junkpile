...
async fn run_listener(bind_addr: String, clients: ClientMap) -> Result<(), String> {
    let listener = TcpListener::bind(&bind_addr).await.map_err(|error| error.to_string())?;
    println!("[junkpile 07] listening on ws://{bind_addr}");
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle_ws(stream, peer_addr, clients).await {
                eprintln!("[junkpile 07] client {peer_addr} error: {error}");
            }
        });
    }
}
...
async fn handle_ws(
...

    {
        ...
        map.insert(
            peer_addr,
            Client { role: "unknown".to_string(), tx: tx.clone() },
        );
    }
    ...
    let reader = tokio::spawn(async move {
        while let Some(message) = ws_rx.next().await {
            match message {
                Ok(Message::Text(text)) => {
                    if let Ok(hello) = serde_json::from_str::<HelloMessage>(&text) {
                        if hello.r#type == "hello" && !hello.role.is_empty() {
                            ...
                            println!("[junkpile 07] {peer_addr} identified as role='{}'", hello.role);
                ...
                Ok(Message::Binary(bytes)) => {
                    ...
                    if sent == 0 {
                        println!("[junkpile 07] binary data arrived before a canvas client connected");
                    }
                }
                ...
                Ok(_) => {}
                Err(error) => {
                    eprintln!("[junkpile 07] receive error from {peer_addr}: {error}");
                    break;
                }
            }
        }
        ...
        broadcast_presence(&reader_clients).await;
        println!("[junkpile 07] {peer_addr} disconnected");
    });
...
fn canvas_window(app: &tauri::AppHandle) -> Result<tauri::Window, String> {
    app.get_window("canvas").ok_or_else(|| "canvas window is unavailable".to_string())
}
...
fn focus_canvas(app: tauri::AppHandle) -> Result<(), String> {
    canvas_window(&app)?.set_focus().map_err(|error| error.to_string())
}
    ...
    tauri::Builder::default()
        .setup(|_app| {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = run_listener(format!("127.0.0.1:{PORT}"), clients_v4).await {
                    eprintln!("[junkpile 07] IPv4 listener error: {error}");
                }
            });
            tauri::async_runtime::spawn(async move {
                if let Err(error) = run_listener(format!("[::1]:{PORT}"), clients_v6).await {
                    eprintln!("[junkpile 07] IPv6 listener error: {error}");
                }
            });
            Ok(())
        })
        ...
        .expect("error while running Junkpile Example 07");
}
