(() => {
  ...
  const LOCAL_KEY = 'junkpile-v1-24-presets';
  ...
  function serializeProject() {
    return {
      format: 'junkpile-tauri-v1-keyframe-automation',
      version: 1,
  ...
  function loadProject(project) {
    if (!project || project.format !== 'junkpile-tauri-v1-keyframe-automation') throw new Error('This is not a Junkpile Example 24 automation project.');
    setPlaying(false);
  ...
  async function saveBytes(bytes, defaultPath, filters) {
    const tauri = window.__TAURI__;
    if (tauri?.dialog?.save && tauri?.fs?.writeBinaryFile) {
      const path = await tauri.dialog.save({ defaultPath, filters });
      if (!path) return false;
      await tauri.fs.writeBinaryFile({ path, contents: bytes });
      return true;
    }
    const blob = new Blob([bytes]);
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a'); link.href = url; link.download = defaultPath; link.click();
  ...
  async function exportProject() {
    ...
    if (await saveBytes(bytes, `${safe}.junkpile-automation.json`, [{ name: 'Junkpile automation', extensions: ['json'] }])) setStatus('Project exported');
  }

  async function importProject() {
    const tauri = window.__TAURI__;
    if (tauri?.dialog?.open && tauri?.fs?.readTextFile) {
      const path = await tauri.dialog.open({ multiple: false, filters: [{ name: 'Junkpile automation', extensions: ['json'] }] });
      if (!path) return;
      loadProject(JSON.parse(await tauri.fs.readTextFile(path)));
    ...
    $('fullscreenBtn').onclick = async () => {
      try {
        if (window.__TAURI__?.window?.appWindow) {
          ...
          await window.__TAURI__.window.appWindow.setFullscreen(!isFullscreen);
        } else if (!document.fullscreenElement) await document.documentElement.requestFullscreen(); else await document.exitFullscreen();
