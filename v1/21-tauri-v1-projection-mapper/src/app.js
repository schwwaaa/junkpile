...
function showSourceControls(kind) {
  ...
  if (kind === "pattern") {
    state.source.kind = "pattern"; state.source.url = ""; state.source.name = ui.patternType.options[ui.patternType.selectedIndex]?.text || "Pattern";
...
async function loadMedia(file) {
  ...
  state.source.kind = kind; state.source.url = state.currentObjectUrl; state.source.name = file.name;
  ui.sourceKind.value = kind; showSourceControls(kind); ui.sourceStatus.textContent = file.name; ui.videoPlay.disabled = kind !== "video"; ui.videoPlay.textContent = "Pause";
  scheduleEmit();
}
...
function loadPresetStore() {
  try { return JSON.parse(localStorage.getItem("junkpile-v1-projection-presets") || "[]"); } catch { return []; }
}

function persistPresets() { localStorage.setItem("junkpile-v1-projection-presets", JSON.stringify(state.presets)); refreshPresetUi(); }
...
function applyPreset(preset) {
  ...
  state.source.kind = "pattern"; state.source.url = ""; state.source.pattern = Number(preset.source?.pattern ?? 0); state.source.speed = Number(preset.source?.speed ?? 1); state.source.fit = preset.source?.fit || "cover"; state.blackout = false;
...
function bindControls() {
  ...
  ui.sourceFile.addEventListener("change", () => loadMedia(ui.sourceFile.files?.[0]).catch((error) => window.alert(error.message || error)));
  ui.clearMedia.addEventListener("click", () => { revokeCurrentUrl(); state.source.kind = "pattern"; state.source.url = ""; ui.sourceKind.value = "pattern"; showSourceControls("pattern"); scheduleEmit(); });
  ...
  ui.showOutput.addEventListener("click", () => sendCommand("show-output")); ui.hideOutput.addEventListener("click", () => sendCommand("hide-output")); ui.snapshotOutput.addEventListener("click", () => sendCommand("snapshot"));
...
async function bindTauriEvents() {
  ...
  await tauriEvent.listen("projection-ready", () => { setOutputOnline(true, "Output connected"); scheduleEmit(); sendCommand("refresh-monitors"); sendCommand("refresh-cameras"); });
  ...
  await tauriEvent.listen("projection-monitors", (event) => {
...
async function initialize() {
  bindControls(); await bindTauriEvents(); refreshPresetUi(); syncStateToControls(); drawMeshEditor();
  scheduleEmit();
