...
async function updateSourceFromPayload() {
  const source = state.payload.source;
  const key = `${source.kind}|${source.path || ""}|${source.url || ""}`;
  if (key === state.mediaKey) {
    if (source.kind === "video") {
      const speed = Math.max(.1, Math.min(3, Number(source.speed) || 1));
      sourceVideo.defaultPlaybackRate = speed;
  ...
  if (source.kind === "image" && (source.path || source.url)) {
    sourceVideo.pause();
    ...
    try {
      let url = source.url || "";
      let revoke = false;
      if (source.path && invoke) {
        const bytes = await invoke("read_image_file", { path: source.path });
        const blob = new Blob([bytes], { type: imageMimeType(source.name || source.path) });
        url = URL.createObjectURL(blob);
        revoke = true;
      }
      if (!url) throw new Error("Image source URL is unavailable");
      ...
      releaseImageUrl();
      if (revoke) state.imageUrl = url;
      state.image = image;
      hudSource.textContent = source.name || "Image";
      emitStatus({ source: source.name || "Image loaded", videoPlaying: false });
    } catch (error) {
      console.error(error);
      state.image = null;
      emitStatus({ message: `Image could not be decoded: ${error.message || error}` });
    }
  } else if (source.kind === "video" && (source.url || source.path)) {
    releaseImageUrl();
    state.image = null;
    const url = source.url || (source.path && convertFileSrc ? convertFileSrc(source.path) : "");
    if (!url) {
      emitStatus({ message: "Video asset URL is unavailable" });
      return;
    }
    sourceVideo.src = url;
    const speed = Math.max(.1, Math.min(3, Number(source.speed) || 1));
    ...
    sourceVideo.play().then(() => {
      ...
      hudSource.textContent = source.name || "Video";
      emitStatus({ source: source.name || "Video", videoPlaying: true });
    }).catch((error) => emitStatus({ message: `Video play blocked: ${error.message || error}` }));
    ...
    state.image = null;
    state.videoPlaying = false;
    hudSource.textContent = source.name || "Generated pattern";
  }
}
...
function drawFrame(now) {
  resizeCanvas(); if (state.meshDirty) rebuildMesh(); void updateSourceFromPayload();
...
async function writeBlobNative(path, blob) {
  ...
  for (let offset = 0; offset < bytes.length; offset += chunkSize) {
    ...
    emitStatus({ message: `Saving PNG… ${Math.min(100, Math.round(((offset + chunkSize) / bytes.length) * 100))}%` });
  }
}
...
async function snapshot() {
  try {
    ...
    if (tauriDialog?.save && invoke) {
      ...
      if (!path) return;
      await writeBlobNative(path, blob);
      emitStatus({ message: `Saved ${path}` });
    } else {
      ...
      emitStatus({ message: "PNG downloaded" });
    }
  } catch (error) {
    emitStatus({ message: `Snapshot failed: ${error.message || error}` });
  }
}
...
window.addEventListener("beforeunload", () => { stopCamera(); releaseImageUrl(); });
