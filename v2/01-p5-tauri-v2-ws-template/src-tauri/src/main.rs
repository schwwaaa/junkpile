...
async fn run_listener(bind_addr: String, clients: ClientMap) -> Result<(), String> {
    ...
    println!("[junkpile v2 01] listening on ws://{bind_addr}");
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle_ws(stream, peer_addr, clients).await {
                eprintln!("[junkpile v2 01] client {peer_addr} error: {error}");
            }
        });
    }
}
...
async fn handle_ws(
    ...
    let reader = tokio::spawn(async move {
        while let Some(message) = ws_rx.next().await {
            match message {
                Ok(Message::Text(text)) => {
                    if let Ok(hello) = serde_json::from_str::<HelloMessage>(&text) {
                        if hello.r#type == "hello" && !hello.role.is_empty() {
                            ...
                            println!(
                                "[junkpile v2 01] {peer_addr} identified as role='{}'",
                                hello.role
                            );
                ...
                Ok(Message::Binary(bytes)) => {
                    ...
                    if sent == 0 {
                        println!(
                            "[junkpile v2 01] binary data arrived before a canvas client connected"
                        );
                    }
                }
                ...
                Ok(_) => {}
                Err(error) => {
                    eprintln!("[junkpile v2 01] receive error from {peer_addr}: {error}");
                    break;
                }
            }
        }
        ...
        broadcast_presence(&reader_clients).await;
        println!("[junkpile v2 01] {peer_addr} disconnected");
    });
...
fn canvas_window(app: &tauri::AppHandle) -> Result<tauri::WebviewWindow, String> {
...
fn show_canvas(app: tauri::AppHandle) -> Result<(), String> {
    ...
    window
        .show()
        .map_err(|error| format!("Could not show canvas window: {error}"))?;
    window
        .set_focus()
        .map_err(|error| format!("Could not focus canvas window: {error}"))
}
...
#[tauri::command]
fn toggle_current_fullscreen(app: tauri::AppHandle) -> Result<bool, String> {
    toggle_canvas_fullscreen(app)
}
...
fn main() {
    tauri::Builder::default()
        .setup(|_app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(error) = run_listener(address, clients_v4).await {
                    eprintln!("[junkpile v2 01] IPv4 listener error: {error}");
                }
            });
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(error) = run_listener(address, clients_v6).await {
                    eprintln!("[junkpile v2 01] IPv6 listener error: {error}");
                }
            });
        ...
        .expect("error while running Junkpile Tauri v2 Example 01");
}
