...
async function invoke(command) {
  if (!window.__TAURI__?.core?.invoke) throw new Error("Tauri API is unavailable");
...
async function toggleOutputFullscreen() {
  try {
    ...
    console.error("[junkpile v2 01] fullscreen failed", error);
  }
}
...
async function showOutput() {
  try {
    ...
    console.error("[junkpile v2 01] show output failed", error);
  }
}
...
async function focusOutput() {
  try {
    ...
    console.error("[junkpile v2 01] focus output failed", error);
  }
}
