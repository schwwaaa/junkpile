(() => {
  ...
  async function saveBlob(blob, suggestedName, extension) {
    if (invoke && tauriDialog?.save) {
      const path = await tauriDialog.save({
        title: extension === "png" ? "Save canvas frame" : "Save canvas recording",
        defaultPath: suggestedName,
        filters: filterForExtension(extension)
      });
      if (!path) return "";
      // Recordings can be hundreds of megabytes. Sending one giant byte array
      // through IPC is wasteful, so native saves are streamed in bounded chunks.
      if (extension !== "png" && blob.size > 1024 * 1024) {
        const chunkSize = 1024 * 1024;
        await invoke("create_binary", { path });
        for (let offset = 0; offset < blob.size; offset += chunkSize) {
          const end = Math.min(blob.size, offset + chunkSize);
          const bytes = Array.from(new Uint8Array(await blob.slice(offset, end).arrayBuffer()));
          await invoke("append_binary", { path, bytes });
          if (ui.saveButton) {
            const percent = Math.round((end / blob.size) * 100);
            ui.saveButton.textContent = `Saving ${percent}%`;
          }
        }
        return path;
      }
      const bytes = Array.from(new Uint8Array(await blob.arrayBuffer()));
      return await invoke("write_binary", { path, bytes });
    }
  ...
  async function saveLastRecording() {
    ...
    const filename = timestampName("junkpile-v2-14-recording", extension);
    try {
  ...
  async function saveSnapshot() {
    try {
      ...
      const filename = timestampName("junkpile-v2-14-frame", "png");
  ...
  async function toggleFullscreen() {
    try {
      ...
      setRecordState("Fullscreen error", "error");
    }
  }
  ...
  function initialize() {
    try {
      ...
      console.error("Tauri v2 Example 14 initialization failed", error);
      unsupported.classList.remove("hidden");
