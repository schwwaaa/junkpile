...
fn inspect_media_file(app: tauri::AppHandle, path: String) -> Result<MediaSelection, String> {
    ...
    // Videos stream through Tauri's asset protocol. Images are returned as raw bytes
    // so WebGL receives origin-clean Blob URLs inside WKWebView.
    if kind == "video" {
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running Junkpile Tauri v2 example 19");
}
