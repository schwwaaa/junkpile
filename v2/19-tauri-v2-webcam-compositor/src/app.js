(() => {
  ...
  const ui = {
    ...
    nativeOpenBackground: $("native-open-background"), backgroundFile: $("background-file"), clearBackground: $("clear-background"), backgroundPlay: $("background-play"),
    ...
    nativeOpenOverlay: $("native-open-overlay"), overlayFile: $("overlay-file"), clearOverlay: $("clear-overlay"), overlayName: $("overlay-name"), overlayBlend: $("overlay-blend"),
    ...
    stopRecording: $("stop-recording"), saveRecording: $("save-recording"), saveSnapshot: $("save-snapshot"), fullscreenPreview: $("fullscreen-preview"),
    ...
    hudView: $("hud-view"), hudKey: $("hud-key"), recordingBorder: $("recording-border"), emptyStart: $("empty-start"), dropOverlay: $("drop-overlay")
  };
  ...
  const VIDEO_EXTENSIONS = ["mp4", "mov", "m4v", "webm", "mkv", "avi", "ogv", "ogg"];
  ...
  const state = {
    ...
    sampleCanvas: document.createElement("canvas"), sampleContext: null, nativeDropInstalled: false, dragDepth: 0
  };
  ...
  function imageMimeType(name) {
    return ({
      ...
      gif: "image/gif", bmp: "image/bmp", tif: "image/tiff", tiff: "image/tiff", avif: "image/avif"
  ...
  async function loadBackgroundSource(name, kind, url, revokeWhenDone) {
    revokeBackgroundUrl();
    if (revokeWhenDone) state.backgroundUrl = url;
    ui.backgroundName.textContent = name || "Background media";
    ...
    if (kind === "video") {
      ...
      await new Promise((resolve, reject) => {
        ...
        backgroundVideo.onerror = () => reject(new Error("Background video could not be decoded by this WebView"));
      });
    ...
    if (kind !== "image") throw new Error("Choose a supported image or video file.");
    ...
    try {
      ...
      throw new Error(`WebGL could not upload the background image: ${error.message || error}`);
    }
  ...
  async function loadBackground(file) {
    ...
    if (kind === "unknown") throw new Error("Choose a WebView-supported image or video file.");
    const url = URL.createObjectURL(file);
    await loadBackgroundSource(file.name, kind, url, true);
  }

  async function loadNativeBackgroundPath(path) {
    if (!invoke) throw new Error("Native media loading requires the Tauri runtime.");
    const selected = await invoke("inspect_media_file", { path });
    if (selected.kind === "image") {
      const bytes = await invoke("read_image_file", { path: selected.path });
      const blob = new Blob([bytes], { type: imageMimeType(selected.name) });
      await loadBackgroundSource(selected.name, "image", URL.createObjectURL(blob), true);
    } else if (selected.kind === "video") {
      if (!convertFileSrc) throw new Error("Tauri asset URL conversion is unavailable.");
      await loadBackgroundSource(selected.name, "video", convertFileSrc(selected.path), false);
    } else {
      throw new Error("The selected file is not supported as a background.");
    }
    ui.recordState.textContent = `Background · ${Math.max(1, Math.round(Number(selected.sizeBytes || 0) / 1024))} KiB`;
  }

  async function openNativeBackground() {
    if (!tauriDialog?.open || !invoke) {
      ui.backgroundFile.click();
      return;
    }
    ui.nativeOpenBackground.disabled = true;
    try {
      const path = await tauriDialog.open({
        multiple: false,
        directory: false,
        title: "Open compositor background",
        filters: [{ name: "Images and videos", extensions: [...IMAGE_EXTENSIONS, ...VIDEO_EXTENSIONS] }]
      });
      if (path && !Array.isArray(path)) await loadNativeBackgroundPath(path);
    } catch (error) {
      console.error(error);
      window.alert(error.message || String(error));
    } finally {
      ui.nativeOpenBackground.disabled = false;
    }
  }
  ...
  function clearBackground() {
    ...
    state.backgroundWidth = 1280;
    state.backgroundHeight = 720;
  ...
  function applyBackgroundSpeed() {
    ...
    const speed = Number(ui.backgroundSpeed.value);
    if (speed <= 0.01) {
      backgroundVideo.pause();
      ui.backgroundPlay.textContent = "Play video";
      return;
    }
    backgroundVideo.playbackRate = Math.min(3, Math.max(0.1, speed));
  }
  ...
  async function loadOverlaySource(name, url, revokeWhenDone) {
    revokeOverlayUrl();
    if (revokeWhenDone) state.overlayUrl = url;
    const image = new Image();
    image.src = url;
    await image.decode();
    ...
    try {
      ...
      throw new Error(`WebGL could not upload the overlay image: ${error.message || error}`);
    }
    ui.overlayName.textContent = name || "Overlay image";
  }

  async function loadOverlay(file) {
    if (!file) return;
    if (inferFileKind(file) !== "image") throw new Error("The overlay must be an image with transparency.");
    await loadOverlaySource(file.name, URL.createObjectURL(file), true);
  }

  async function loadNativeOverlayPath(path) {
    if (!invoke) throw new Error("Native image loading requires the Tauri runtime.");
    const selected = await invoke("inspect_media_file", { path });
    if (selected.kind !== "image") throw new Error("The overlay must be an image file.");
    const bytes = await invoke("read_image_file", { path: selected.path });
    const blob = new Blob([bytes], { type: imageMimeType(selected.name) });
    await loadOverlaySource(selected.name, URL.createObjectURL(blob), true);
    ui.recordState.textContent = `Overlay · ${Math.max(1, Math.round(Number(selected.sizeBytes || 0) / 1024))} KiB`;
  }

  async function openNativeOverlay() {
    if (!tauriDialog?.open || !invoke) {
      ui.overlayFile.click();
      return;
    }
    ui.nativeOpenOverlay.disabled = true;
    try {
      const path = await tauriDialog.open({
        multiple: false,
        directory: false,
        title: "Open transparent overlay image",
        filters: [{ name: "Images", extensions: IMAGE_EXTENSIONS }]
      });
      if (path && !Array.isArray(path)) await loadNativeOverlayPath(path);
    } catch (error) {
      console.error(error);
      window.alert(error.message || String(error));
    } finally {
      ui.nativeOpenOverlay.disabled = false;
    }
  }
  ...
  function clearOverlay() {
    ...
    state.overlayWidth = generated.width;
    state.overlayHeight = generated.height;
    state.overlayGenerated = true;
    uploadTexture(state.overlayTexture, generated, 2);
    ui.overlayName.textContent = "Generated frame";
  }

  function dropTargetAtPosition(position) {
    if (!position) return "background";
    const rawX = Number(position.x);
    const rawY = Number(position.y);
    if (!Number.isFinite(rawX) || !Number.isFinite(rawY)) return "background";
    const ratio = Math.max(1, window.devicePixelRatio || 1);
    for (const [x, y] of [[rawX / ratio, rawY / ratio], [rawX, rawY]]) {
      const section = document.elementFromPoint(x, y)?.closest?.("[data-drop-target]");
      if (section?.dataset?.dropTarget) return section.dataset.dropTarget;
    }
    return "background";
  }

  function setDropHighlight(target, active) {
    document.querySelectorAll("[data-drop-target]").forEach((element) => {
      element.classList.toggle("dragging", active && element.dataset.dropTarget === target);
    });
  ...
  async function installNativeDropHandler() {
    ...
    try {
      ...
      await currentWebview.onDragDropEvent((event) => {
        ...
        const target = dropTargetAtPosition(payload.position);
        ...
        if (payload.type === "drop") {
          ...
          const path = Array.from(payload.paths || []).find((candidate) => inferPathKind(candidate) !== "unknown");
          if (!path) {
            ui.recordState.textContent = "No supported media in drop";
            return;
          }
          const task = target === "overlay" ? loadNativeOverlayPath(path) : loadNativeBackgroundPath(path);
          void task.catch((error) => {
            console.error(error);
            ui.recordState.textContent = "Drop failed";
            window.alert(error.message || String(error));
          });
          return;
        }
      ...
      console.error("Could not install native drag/drop", error);
      ui.recordState.textContent = "Native drop unavailable";
    }
  }

  function configureBrowserDropTarget(element, target) {
    ...
    element.addEventListener("dragleave", (event) => {
      if (!event.relatedTarget || !element.contains(event.relatedTarget)) element.classList.remove("dragging");
    });
    element.addEventListener("drop", (event) => {
      event.preventDefault();
      element.classList.remove("dragging");
      if (state.nativeDropInstalled) return;
      const file = event.dataTransfer?.files?.[0];
      const task = target === "overlay" ? loadOverlay(file) : loadBackground(file);
      void task.catch((error) => window.alert(error.message || String(error)));
    });
  }
  ...
  async function writeBlobNative(path, blob) {
    ...
    for (let offset = 0; offset < blob.size; offset += chunkSize) {
      ...
      written += chunk.size;
      ui.recordState.textContent = `Writing ${Math.round((written / blob.size) * 100)}%`;
    }
    return path;
  }
  ...
  async function saveBlob(blob, suggestedName, extension) {
    if (invoke && tauriDialog?.save) {
      const path = await tauriDialog.save({
        ...
        title: extension === "png" ? "Save composited frame" : "Save compositor recording",
  ...
  function bindControls() {
    ...
    ui.nativeOpenBackground.addEventListener("click", openNativeBackground);
    ui.backgroundFile.addEventListener("change", () => {
      loadBackground(ui.backgroundFile.files?.[0]).catch((error) => window.alert(error.message || error));
    ...
    ui.backgroundSpeed.addEventListener("input", applyBackgroundSpeed);
    ui.nativeOpenOverlay.addEventListener("click", openNativeOverlay);
    ui.overlayFile.addEventListener("change", () => {
      loadOverlay(ui.overlayFile.files?.[0]).catch((error) => window.alert(error.message || error));
    ...
    ui.fullscreenPreview.addEventListener("click", async () => {
      if (invoke) {
        try {
          const enabled = await invoke("toggle_fullscreen");
          ui.fullscreenPreview.textContent = enabled ? "Exit fullscreen" : "Fullscreen preview";
          return;
        } catch (error) {
          console.error(error);
        }
    ...
    document.querySelectorAll("[data-drop-target]").forEach((element) => configureBrowserDropTarget(element, element.dataset.dropTarget));
