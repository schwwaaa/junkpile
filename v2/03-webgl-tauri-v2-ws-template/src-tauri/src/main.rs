...
async fn run_listener(bind_address: String, clients: ClientMap) -> Result<(), String> {
    ...
    println!("[junkpile-03] listening on ws://{bind_address}");
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle_websocket(stream, peer_address, clients).await {
                eprintln!("[junkpile-03] client {peer_address} error: {error}");
            }
        });
    }
}
...
async fn handle_websocket(
    ...
    println!("[junkpile-03] {peer_address} connected");
    ...
    let reader_task = tokio::spawn(async move {
        while let Some(result) = socket_reader.next().await {
            match result {
                    ...
                    if let Ok(hello) = serde_json::from_str::<HelloMessage>(&text) {
                        if hello.r#type == "hello" && !hello.role.is_empty() {
                            ...
                            if let Some(client) = map.get_mut(&peer_address) {
                                ...
                                println!(
                                    "[junkpile-03] {peer_address} identified as '{}'",
                                    client.role
                                );
                            }
                        }
                ...
                Ok(Message::Binary(bytes)) => {
                    ...
                    if recipients == 0 {
                        println!(
                            "[junkpile-03] binary message from {peer_address} had no canvas recipient"
                        );
                    }
                }
                ...
                Ok(_) => {}
                Err(error) => {
                    eprintln!("[junkpile-03] receive error from {peer_address}: {error}");
                    break;
                }
            }
        }
        ...
        println!("[junkpile-03] {peer_address} disconnected");
    });
...
fn main() {
    tauri::Builder::default()
        .setup(|_app| {
            println!("[junkpile-03] starting embedded WebSocket relay on port {PORT}");
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(error) = run_listener(address, ipv4_clients).await {
                    eprintln!("[junkpile-03] IPv4 listener error: {error}");
                }
            });
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(error) = run_listener(address, ipv6_clients).await {
                    eprintln!("[junkpile-03] IPv6 listener error: {error}");
                }
            });
