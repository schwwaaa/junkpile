(() => {
  ...
  const ui = {
    ...
    logoOpen: $("logo-open"), logoFile: $("logo-file"), clearLogo: $("clear-logo"), viewMode: $("view-mode"), masterFade: $("master-fade"), masterFadeOutput: $("master-fade-output"),
    ...
    fullscreenPreview: $("fullscreen-preview"), dropOverlay: $("drop-overlay"), dropTargetLabel: $("drop-target-label"),
  ...
  const sourceUi = Array.from({ length: 4 }, (_, index) => ({
    ...
    card: document.querySelector(`[data-source="${index}"]`), open: document.querySelector(`[data-open="${index}"]`),
  ...
  const state = {
    ...
    nativeDropInstalled: false, dropTarget: 0, dragDepth: 0
  };
  ...
  function initializeSources() {
    state.sources = Array.from({ length: 4 }, (_, index) => {
      ...
      return { kind: "pattern", element: patternCanvas, name: patternNames[index], width: 640, height: 360, url: "", revokeUrl: false, playing: true, patternIndex: index, speed: 1 };
    });
    refreshAllSourceUi();
  }
  ...
  function revokeSourceUrl(index) {
    ...
    if (source?.url && source.revokeUrl) URL.revokeObjectURL(source.url);
    if (source) { source.url = ""; source.revokeUrl = false; }
  }
  ...
  function resetSourceToPattern(index) {
    ...
    state.sources[index] = { kind: "pattern", element: patternCanvas, name: patternNames[index], width: patternCanvas.width, height: patternCanvas.height, url: "", revokeUrl: false, playing: true, patternIndex: index, speed };
  ...
  async function loadSourceFile(index, file) {
    ...
    if (file.type.startsWith("video/") || /\.(mp4|mov|m4v|webm|ogv)$/i.test(file.name)) {
      ...
      state.sources[index] = { kind: "video", element: video, name: file.name, width: video.videoWidth, height: video.videoHeight, url, revokeUrl: true, playing: !video.paused, patternIndex: index, speed };
    } else if (file.type.startsWith("image/") || /\.(png|jpe?g|webp|gif|bmp)$/i.test(file.name)) {
      const image = new Image();
      image.src = url;
      await image.decode();
      state.sources[index] = { kind: "image", element: image, name: file.name, width: image.naturalWidth, height: image.naturalHeight, url, revokeUrl: true, playing: false, patternIndex: index, speed: state.sources[index]?.speed || 1 };
    } else {
  ...
  async function loadNativeSource(index, path, origin = "native") {
    ...
    if (selected.kind === "image") {
      ...
      await image.decode();
      state.sources[index] = {
        kind: "image", element: image, name: selected.name,
        width: image.naturalWidth, height: image.naturalHeight,
        url, revokeUrl: true, playing: false, patternIndex: index, speed
      };
    } else {
      if (!convertFileSrc) throw new Error("Tauri asset URL conversion is unavailable.");
      const video = document.createElement("video");
      video.src = convertFileSrc(selected.path);
      video.loop = true;
      video.muted = true;
      video.playsInline = true;
      video.preload = "auto";
      await new Promise((resolve, reject) => {
        video.addEventListener("loadeddata", resolve, { once: true });
        video.addEventListener("error", () => reject(new Error(`Could not decode ${selected.name}`)), { once: true });
      });
      video.defaultPlaybackRate = speed;
      video.playbackRate = speed;
      await video.play().catch(() => {});
      state.sources[index] = {
        kind: "video", element: video, name: selected.name,
        width: video.videoWidth, height: video.videoHeight,
        url: video.src, revokeUrl: false, playing: !video.paused, patternIndex: index, speed
      };
    }
    ui.recordState.textContent = `${origin} → source ${index + 1}`;
    refreshSourceUi(index);
  }

  async function openSourceDialog(index) {
    if (!tauriDialog?.open || !invoke) {
      sourceUi[index].file.click();
      return;
    }
    const path = await tauriDialog.open({
      title: `Open media for source ${index + 1}`,
      multiple: false,
      directory: false,
      filters: [{ name: "Images and videos", extensions: ["png","jpg","jpeg","webp","gif","bmp","tif","tiff","avif","mp4","mov","m4v","webm","mkv","avi","ogv","ogg"] }]
    });
    if (typeof path === "string") await loadNativeSource(index, path, "opened");
  }

  function setDropTarget(index) {
    state.dropTarget = Math.max(0, Math.min(3, Number(index) || 0));
    sourceUi.forEach((panel, panelIndex) => panel.card?.classList.toggle("drop-target", panelIndex === state.dropTarget));
    if (ui.dropTargetLabel) ui.dropTargetLabel.textContent = `DROP TO SOURCE ${state.dropTarget + 1}`;
  }
  ...
  function handleBrowserDrop(event) {
    ...
    const file = Array.from(event.dataTransfer?.files || []).find((item) => item.type.startsWith("image/") || item.type.startsWith("video/") || isImagePath(item.name) || isVideoPath(item.name));
    if (file) void loadSourceFile(state.dropTarget, file).catch((error) => window.alert(error.message || error));
  }
    ...
    try {
      ...
      await currentWebview.onDragDropEvent((event) => {
        ...
        if (payload.type === "drop") {
          ...
          void loadNativeSource(state.dropTarget, path, "dropped").catch((error) => {
            console.error(error);
  ...
  function stopCamera() {
    ...
    if (oldSlot >= 0 && state.sources[oldSlot]?.kind === "camera") {
      ...
      state.sources[oldSlot] = { kind: "pattern", element: patternCanvas, name: patternNames[oldSlot], width: patternCanvas.width, height: patternCanvas.height, url: "", revokeUrl: false, playing: true, patternIndex: oldSlot, speed: state.sources[oldSlot]?.speed || 1 };
  ...
  async function assignCamera(index) {
    ...
    if (state.cameraSlot >= 0 && state.cameraSlot !== index && state.sources[state.cameraSlot]?.kind === "camera") {
      ...
      state.sources[previous] = { kind: "pattern", element: patternCanvas, name: patternNames[previous], width: patternCanvas.width, height: patternCanvas.height, url: "", revokeUrl: false, playing: true, patternIndex: previous, speed: state.sources[previous]?.speed || 1 };
    ...
    try {
      ...
      state.sources[index] = { kind: "camera", element: cameraVideo, name: "Live Camera", width: cameraVideo.videoWidth || 1280, height: cameraVideo.videoHeight || 720, url: "", revokeUrl: false, playing: true, patternIndex: index, speed: state.sources[index]?.speed || 1 };
  ...
  async function loadLogoUrl(url, revokeOnReplace = true) {
    if (state.logoUrl) URL.revokeObjectURL(state.logoUrl);
    state.logoUrl = revokeOnReplace ? url : "";
    const image = new Image();
    image.src = url;
    await image.decode();
  ...
  async function loadLogo(file) {
    if (!file) return;
    await loadLogoUrl(URL.createObjectURL(file), true);
  }

  async function openNativeLogo() {
    if (!tauriDialog?.open || !invoke) {
      ui.logoFile.click();
      return;
    }
    const path = await tauriDialog.open({
      ...
      multiple: false,
      directory: false,
      filters: [{ name: "Images", extensions: ["png","jpg","jpeg","webp","gif","bmp","tif","tiff","avif"] }]
    });
    if (typeof path !== "string") return;
    const selected = await invoke("inspect_media_file", { path });
    if (selected.kind !== "image") throw new Error("Choose an image for the logo.");
    const bytes = await invoke("read_image_file", { path: selected.path });
    const blob = new Blob([bytes], { type: imageMimeType(selected.name) });
    await loadLogoUrl(URL.createObjectURL(blob), true);
  }
  ...
  async function saveBlob(blob, suggestedName, extension) {
    if (invoke && tauriDialog?.save) {
      const path = await tauriDialog.save({
        ...
        title: extension === "png" ? "Save program snapshot" : "Save program recording",
  ...
  function bindControls() {
    sourceUi.forEach((panel, index) => {
      panel.open.addEventListener("click", () => openSourceDialog(index).catch((error) => window.alert(error.message || error)));
      panel.card.addEventListener("pointerenter", () => setDropTarget(index));
      panel.card.addEventListener("focusin", () => setDropTarget(index));
      panel.card.addEventListener("click", () => setDropTarget(index));
    ...
    ui.logoOpen.addEventListener("click", () => openNativeLogo().catch((error) => window.alert(error.message || error)));
    ...
    stage.addEventListener("dragenter", (event) => { event.preventDefault(); setDropOverlay(true); });
    stage.addEventListener("dragover", (event) => { event.preventDefault(); setDropOverlay(true); });
    stage.addEventListener("dragleave", () => { if (!state.nativeDropInstalled) setDropOverlay(false); });
    stage.addEventListener("drop", handleBrowserDrop);
  ...
  function initialize() {
    try {
      ...
      console.error("Tauri v2 Example 20 initialization failed", error);
