...
// =============================================================================
// Junkpile 13 — Tauri v2 Audio-Reactive FFT
//
// Signal path:
...
const clearFeedbackButton = $('clear-feedback');
...
function clearFeedback() {
  if (!feedbackFramebuffers.some(Boolean)) return;
  gl.clearColor(0, 0, 0, 1);
  feedbackFramebuffers.forEach((framebuffer) => {
    gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
    gl.clear(gl.COLOR_BUFFER_BIT);
  ...
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  setSaveStatus('Trails cleared', 'success');
}
...
function canvasToPngBytes() {
  return new Promise((resolve, reject) => {
    canvas.toBlob(async (blob) => {
      if (!blob) {
        reject(new Error('The WebGL canvas could not be encoded as PNG'));
        return;
      }
...
async function saveSnapshot() {
  ...
  setSaveStatus('Preparing PNG…');
  try {
    const bytes = await canvasToPngBytes();
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `junkpile-v2-audio-fft-${stamp}.png`;

    if (invoke && tauriDialog?.save) {
      const path = await tauriDialog.save({
        title: 'Save audio-reactive frame',
      ...
      setSaveStatus(`Saved ${savedPath.split(/[\\/]/).pop()}`, 'success');
      return;
    }
    ...
    setSaveStatus('Downloaded PNG', 'success');
  } catch (error) {
    console.error('Snapshot failed', error);
    setSaveStatus(error?.message || 'Snapshot failed', 'error');
  } finally {
...
async function toggleFullscreen() {
  try {
    ...
    setSaveStatus(error?.message || 'Fullscreen failed', 'error');
  }
}
