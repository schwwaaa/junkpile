...
// Junkpile Example 09 — Feedback Lab WebSocket Renderer
//
// This WebView owns camera permission, decoded video frames, the camera texture,
// two persistent framebuffer targets, both GLSL programs, pointer brush input,
// and the render loop. Only JSON controls and telemetry cross the local socket.
...
let lastDevices = [];
let lastCameraState = { state: "idle", help: "Camera input is idle." };
...
function refreshHudRuntime() {
  const suffix = wsConnected ? "" : " · WS OFFLINE";
  elements.hudRuntime.textContent = `${lastRuntime.text}${suffix}`.toUpperCase();
}
...
function setDiagnostic(id, text, result = "") {
  if (!DIAGNOSTIC_IDS.includes(id)) return;
  ...
  if (id === "target-format") elements.hudFormat.textContent = text.toUpperCase();
  send({ type: "diagnostic", id, text, result });
}

function replayTelemetry() {
  send({ type: "runtime", ...lastRuntime });
  for (const [id, data] of Object.entries(diagnostics)) {
    send({ type: "diagnostic", id, text: data.text, result: data.result });
  }
  send({ type: "error-log", scope: lastError.scope, message: lastError.message });
  send({ type: "paused-state", paused });
  send({ type: "camera-status", ...lastCameraState, activeDeviceId });
  send({ type: "camera-devices", devices: lastDevices, activeDeviceId });
}
...
function chooseFeedbackType(width, height) {
  if (gl.getExtension("OES_texture_float")) {
    ...
    if (trial) {
      ...
      return {
        type: gl.FLOAT,
        name: gl.getExtension("OES_texture_float_linear") ? "RGBA32F linear" : "RGBA32F nearest",
      };
    }
  }
...
function renderDisplayPass() {
  if (!framebufferTextures[writeTarget]) return;
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  gl.viewport(0, 0, elements.canvas.width, elements.canvas.height);
...
function render(now) {
  ...
  try {
    ...
    failRenderer("The feedback render loop stopped.", error instanceof Error ? error.message : String(error));
  }
}
...
async function enumerateCameras(preferred = "") {
  ...
  try {
    ...
    const cameras = devices
      ...
      .map((device, index) => ({
        ...
        label: device.label || `Camera ${index + 1} — permission required`,
      }));
    lastDevices = cameras;
    if (preferred && cameras.some((device) => device.deviceId === preferred)) preferredDeviceId = preferred;
    send({ type: "camera-devices", devices: cameras, activeDeviceId });
    if (!cameras.length && !cameraStream) setDiagnostic("camera-state", "No devices", "warning");
    return cameras;
  } catch (error) {
    const message = `Could not enumerate camera devices. ${error instanceof Error ? error.message : String(error)}`;
    lastDevices = [];
...
function describeCameraError(error) {
  ...
  switch (name) {
    ...
    case "PermissionDeniedError":
      return "Camera access was denied. Allow access in system privacy settings, then start the camera again.";
    case "NotFoundError":
...
function sendCameraStatus(state, help = "") {
  lastCameraState = { state, help };
  send({ type: "camera-status", state, activeDeviceId, help });
}

async function startCamera(deviceId = "") {
  if (cameraBusy) return;
  if (cameraStream && (!deviceId || deviceId === activeDeviceId)) {
    sendCameraStatus("running", "The requested camera is already active in the renderer window.");
  ...
  if (!mediaDevicesAvailable()) {
    ...
    setDiagnostic("camera-state", "Unsupported", "error");
    showCameraNotice("Camera API unavailable", message, "error");
    sendCameraStatus("error", message);
    return;
  }

  cameraBusy = true;
  preferredDeviceId = deviceId || preferredDeviceId;
  sendCameraStatus("requesting", "Approve the camera permission prompt in the renderer window.");
  ...
  try {
    ...
    preferredDeviceId = activeDeviceId || preferredDeviceId;
    lastUploadedVideoTime = -1;
    uploadMetricFrames = 0;
    uploadMetricStart = performance.now();
    track.addEventListener("ended", () => {
      if (cameraStream?.getVideoTracks()[0] === track) stopCamera("The active camera track ended.", true);
    }, { once: true });
...
function stopCamera(message = "Camera stopped. Draw on the renderer or restart camera input.", endedUnexpectedly = false) {
  ...
  setDiagnostic("camera-name", "—");
  setDiagnostic("camera-resolution", "—");
  setDiagnostic("upload-fps", "0.0 fps");
  ...
  sendCameraStatus(endedUnexpectedly ? "ended" : "stopped", message);
  void enumerateCameras(preferredDeviceId);
}

function updateModeLabel() {
  elements.hudMode.textContent = (MODE_NAMES[params.mode] || MODE_NAMES[0]).toUpperCase();
}

function selectMode(mode, notifyControls = false) {
  const nextMode = Math.max(0, Math.min(MODE_NAMES.length - 1, Number(mode)));
  if (params.mode !== nextMode) {
    params.mode = nextMode;
    updateModeLabel();
    clearFeedbackState();
  }
  if (notifyControls) send({ type: "renderer-state", params: { ...params }, paused });
}

function applyState(message) {
  const previousMode = params.mode;
  if (message.params && typeof message.params === "object") {
    for (const [name, value] of Object.entries(message.params)) {
      ...
      params[name] = name === "palette" || name === "mode" ? Math.round(numericValue) : numericValue;
    }
  }
  params.mode = Math.max(0, Math.min(MODE_NAMES.length - 1, params.mode));
  params.palette = Math.max(0, Math.min(PALETTE_NAMES.length - 1, params.palette));
  updateModeLabel();
  if (params.mode !== previousMode) clearFeedbackState();
...
function resetRenderer(notifyControls = true) {
...
async function toggleFullscreen() {
  try {
    ...
    showErrorLog("tauri", `Fullscreen request failed. ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function handleMessage(message) {
  ...
  if (message.type === "hello" && message.role === "controls") {
    send({ type: "canvas-ready" });
    send({ type: "request-state", role: "canvas" });
    replayTelemetry();
    void enumerateCameras(activeDeviceId || preferredDeviceId);
    return;
  }

  if (message.type === "param" && Object.prototype.hasOwnProperty.call(DEFAULTS, message.name)) {
    const value = Number(message.value);
    if (!Number.isFinite(value)) return;
    params[message.name] = message.name === "palette" || message.name === "mode" ? Math.round(value) : value;
    return;
  }
  ...
  if (message.type === "camera-command") {
    if (message.action === "start") void startCamera(message.deviceId || "");
    else if (message.action === "stop") stopCamera();
    else if (message.action === "enumerate") void enumerateCameras(activeDeviceId || preferredDeviceId);
    return;
  }

  if (message.type === "action") {
    if (message.name === "set-paused") setPaused(Boolean(message.value));
    else if (message.name === "set-mode") selectMode(message.value);
    else if (message.name === "clear-feedback") clearFeedbackState();
    else if (message.name === "reset-feedback") {
      shaderTime = 0;
      clearFeedbackState();
  ...
  if (message.type === "request-state" && message.role !== "canvas") {
    send({ type: "canvas-ready" });
    replayTelemetry();
    void enumerateCameras(activeDeviceId || preferredDeviceId);
  }
...
function connect() {
  ...
  ws.addEventListener("message", (event) => {
    ...
    try {
      ...
      // The local relay broadcasts raw text; malformed messages are ignored.
    }
  });
...
function wireBrush() {
  ...
  const end = (event) => {
    ...
    pointer.down = false;
    pointer.id = null;
    delete elements.rendererPanel.dataset.brushing;
  };
...
function wireLifecycleEvents() {
  elements.canvas.addEventListener("webglcontextlost", (event) => {
    ...
    contextLost = true;
    failRenderer("The WebGL context was lost.", "Reload the renderer to rebuild the camera texture and persistent feedback targets.");
  });
  elements.canvas.addEventListener("webglcontextrestored", () => window.location.reload());
  if (navigator.mediaDevices?.addEventListener) {
    navigator.mediaDevices.addEventListener("devicechange", () => void enumerateCameras(activeDeviceId || preferredDeviceId));
  }
  ...
  window.addEventListener("keydown", (event) => {
    ...
    else if (key === "c") cameraStream ? stopCamera() : void startCamera(preferredDeviceId);
    else if (key === "f") void toggleFullscreen();
    else if (/^[1-6]$/.test(key)) selectMode(Number(key) - 1, true);
  });
}
...
async function start() {
  ...
  wireBrush();
  wireLifecycleEvents();
  setDiagnostic("camera-state", "Idle");
  setDiagnostic("camera-name", "—");
  setDiagnostic("camera-resolution", "—");
  setDiagnostic("upload-fps", "0.0 fps");
  ...
  new ResizeObserver(() => {
    try {
      ...
      failRenderer("The feedback targets could not be resized.", error instanceof Error ? error.message : String(error));
    }
  }).observe(elements.rendererPanel);
