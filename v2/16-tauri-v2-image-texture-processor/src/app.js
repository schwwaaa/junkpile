(() => {
  ...
  function isImageFile(file) {
    return Boolean(file && (file.type?.startsWith("image/") || isImagePath(file.name)));
  }

  function loadImageUrl(url, name, revokeOnReplace = false) {
    ui.sourceState.textContent = "Loading…";
    if (state.objectUrl) URL.revokeObjectURL(state.objectUrl);
    state.objectUrl = revokeOnReplace ? url : null;
    const image = new Image();
    image.onload = () => {
      try {
        uploadSource(image, image.naturalWidth, image.naturalHeight, name);
        ui.exportState.textContent = "Ready";
      } catch (error) {
        console.error("Could not upload image to WebGL", error);
        ui.sourceState.textContent = "GPU upload failed";
        ui.exportState.textContent = "WebGL rejected image";
        window.alert(`The image decoded, but WebGL could not use it:
${error.message || error}`);
      }
    };
    image.onerror = () => {
      ui.sourceState.textContent = "Load failed";
      ui.exportState.textContent = "Decode failed";
      window.alert("The WebView could not decode that image. Try PNG, JPEG, or WebP.");
      if (revokeOnReplace) {
  ...
  async function loadNativePath(path, origin = "native") {
    if (!invoke) throw new Error("Native image loading requires Tauri.");
    if (!isImagePath(path)) throw new Error("The selected path does not look like a supported image.");
    ui.sourceState.textContent = "Reading…";
    const selected = await invoke("inspect_image_file", { path });
    ...
    ui.exportState.textContent = `${origin} image · ${Math.max(1, Math.round(Number(selected.sizeBytes || 0) / 1024))} KiB`;
    loadImageUrl(objectUrl, selected.name, true);
  }
    ...
    try {
      ...
      await currentWebview.onDragDropEvent((event) => {
        ...
        if (payload.type === "drop") {
          setDragState(false);
          const path = Array.from(payload.paths || []).find(isImagePath);
          if (!path) {
            ui.exportState.textContent = "No supported image in drop";
            return;
          }
          ui.exportState.textContent = "Opening dropped image…";
          void loadNativePath(path, "dropped").catch((error) => {
            console.error(error);
            ui.sourceState.textContent = "Drop failed";
      ...
      console.error("Could not install native image drop handler", error);
  ...
  function initializeControls() {
    ...
    ui.nativeOpen.addEventListener("click", async () => {
      if (!invoke || !tauriDialog?.open) {
        ui.imageInput.click();
        return;
      }
      ui.nativeOpen.disabled = true;
      ui.exportState.textContent = "Opening dialog…";
      try {
        const path = await tauriDialog.open({
          multiple: false,
          directory: false,
          title: "Open an image texture",
        ...
        console.error(error);
        ui.sourceState.textContent = "Open failed";
        ui.exportState.textContent = "Open failed";
        window.alert(error.message || String(error));
      } finally {
        ui.nativeOpen.disabled = false;
      }
    });
    ui.imageInput.addEventListener("change", () => {
      loadFile(ui.imageInput.files?.[0]);
      ui.imageInput.value = "";
    });
  ...
  function start() {
    try {
      ...
      console.error("Could not initialize Tauri v2 Example 16", error);
