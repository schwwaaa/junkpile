(() => {
  ...
  const ui = {
    nativeOpenA: $("native-open-a"),
    nativeOpenB: $("native-open-b"),
  ...
  function loadImageFile(key, file, objectUrl, revocable = true) {
    const image = new Image();
    image.decoding = "async";
    image.onload = () => {
      setLayerElement(key, image, image.naturalWidth, image.naturalHeight, file.name, "image", revocable ? objectUrl : null);
    };
    image.onerror = () => {
  ...
  function loadVideoFile(key, file, objectUrl, revocable = true) {
    ...
    video.addEventListener("loadedmetadata", () => {
      setLayerElement(key, video, video.videoWidth, video.videoHeight, file.name, "video", revocable ? objectUrl : null);
  ...
  function loadFile(key, file) {
    ...
    if (type === "video") loadVideoFile(key, file, objectUrl, true);
    else loadImageFile(key, file, objectUrl, true);
  }

  async function loadNativePath(key, path, origin = "native") {
    ...
    const metadata = { name: selected.name };
    if (selected.kind === "image") {
      ...
      loadImageFile(key, metadata, URL.createObjectURL(blob), true);
    } else if (selected.kind === "video") {
      if (!convertFileSrc) throw new Error("Tauri asset URL conversion is unavailable.");
      loadVideoFile(key, metadata, convertFileSrc(selected.path), false);
    } else {
      throw new Error("The selected file is not a supported image or video.");
    }
    ui.exportState.textContent = `${origin} → Layer ${key.toUpperCase()} · ${Math.max(1, Math.round(Number(selected.sizeBytes || 0) / 1024))} KiB`;
  }

  async function openNativeLayer(key) {
    if (!invoke || !tauriDialog?.open) {
      (key === "a" ? ui.sourceAInput : ui.sourceBInput).click();
      return;
    }
    const button = key === "a" ? ui.nativeOpenA : ui.nativeOpenB;
    button.disabled = true;
    ui.exportState.textContent = `Opening Layer ${key.toUpperCase()}…`;
    try {
      const path = await tauriDialog.open({
        multiple: false,
        directory: false,
        title: `Open Layer ${key.toUpperCase()} media`,
        filters: [{ name: "Images and videos", extensions: [...IMAGE_EXTENSIONS, ...VIDEO_EXTENSIONS] }]
      });
      if (!path || Array.isArray(path)) {
        ui.exportState.textContent = "Open cancelled";
        return;
      }
      await loadNativePath(key, path, "native");
    } catch (error) {
      console.error(error);
      ui.exportState.textContent = "Open failed";
  ...
  function setGlobalDropState(active) {
    ui.dropOverlay.classList.toggle("hidden", !active);
  }

  function layerAtNativePosition(position) {
    if (!position) return state.activeLayer;
    const rawX = Number(position.x);
    const rawY = Number(position.y);
    if (!Number.isFinite(rawX) || !Number.isFinite(rawY)) return state.activeLayer;
    const ratio = Math.max(1, window.devicePixelRatio || 1);
    const candidates = [[rawX / ratio, rawY / ratio], [rawX, rawY]];
    for (const [x, y] of candidates) {
      const element = document.elementFromPoint(x, y);
      const card = element?.closest?.("[data-layer]");
      if (card?.dataset?.layer) return card.dataset.layer;
    }
    ...
    try {
      ...
      await currentWebview.onDragDropEvent((event) => {
        ...
        if (payload.type === "over" || payload.type === "enter") {
          setGlobalDropState(true);
          return;
        }
        if (payload.type === "drop") {
          setGlobalDropState(false);
          const path = Array.from(payload.paths || []).find((candidate) => inferPathType(candidate) !== "unknown");
          if (!path) {
            ui.exportState.textContent = "No supported media in drop";
            return;
          }
          const key = layerAtNativePosition(payload.position);
          setActiveLayer(key);
          ui.exportState.textContent = `Opening drop in Layer ${key.toUpperCase()}…`;
          void loadNativePath(key, path, "dropped").catch((error) => {
            console.error(error);
            ui.exportState.textContent = "Drop failed";
            window.alert(error.message || String(error));
          });
          return;
        }
      ...
      console.error("Could not install native media drop handler", error);
      ui.exportState.textContent = "Native drop unavailable";
    }
  }
  ...
  async function writeBlobNative(path, blob) {
    ...
    for (let offset = 0; offset < blob.size; offset += chunkSize) {
      ...
      written += chunk.size;
      ui.exportState.textContent = `Writing ${Math.round((written / blob.size) * 100)}%`;
    }
    return path;
  }
  ...
  function initializeControls() {
    ...
    ui.nativeOpenA.addEventListener("click", () => openNativeLayer("a"));
    ui.nativeOpenB.addEventListener("click", () => openNativeLayer("b"));
    ui.sourceAInput.addEventListener("change", () => {
      loadFile("a", ui.sourceAInput.files?.[0]);
      ui.sourceAInput.value = "";
    });
    ui.sourceBInput.addEventListener("change", () => {
      loadFile("b", ui.sourceBInput.files?.[0]);
      ui.sourceBInput.value = "";
    });
    ...
    stage.addEventListener("drop", (event) => {
      ...
      const file = Array.from(event.dataTransfer?.files || []).find((candidate) => inferMediaType(candidate) !== "unknown");
      if (file) loadFile(state.activeLayer, file);
    });
  ...
  function start() {
    try {
      ...
      console.error("Could not initialize Tauri v2 Example 18", error);
